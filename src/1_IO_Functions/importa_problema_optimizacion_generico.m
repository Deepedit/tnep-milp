function importa_problema_optimizacion_generico(data, sep, pAdmProy, pAdmSc, pParOpt)
...
function importa_generadores(data, param, sep, pAdmSc, pAdmProy, se_aisladas)
    ...
    for i = 1:ng
        ...
        if evol_capacidad
            ...
            gen.IndiceAdmEscenarioCapacidad = zeros(1,param.CantidadEscenarios);
        end
        if evol_costos
            gen.inserta_evolucion_costos_a_futuro(true);
            gen.IndiceAdmEscenarioCostosFuturos = zeros(1,param.CantidadEscenarios);
        end
        % falta retiro proyectado
		...
		if existente
			...
			gen.Existente = 0;
            gen.Proyectado = 1; % para diferenciar de decisión de expansión de la generación
%			cant_escenarios_validos = 0;
			for escenario = 1:param.CantidadEscenarios
                id_escenario = data.Escenarios(escenario,1);
				fila_generador = find(ismember(data.EvolucionCapacidadGeneradores(:,1:2), [id_escenario id_gen],'rows'));
...
%        throw(error)
    ...
    for lpar = 1:nmax
        ...
        if con_compensacion_serie && Corredores(id_corr,6) >= lpar
        ...
        if con_voltage_uprating && lpar <= Corredores(id_corr,5)
            ...
            if vu_con_conductor_actual
                for i = 1:length(voltajes_vu)
                    ...
                    se1_vur = pAdmProy.entrega_subestacion_por_nombre(nombre_bus1, false);
                    se2_vur = pAdmProy.entrega_subestacion_por_nombre(nombre_bus2, false);
            ...
            % se generan los transformadores en ambos lados si aún no se han creado.
            ubicacion_1 = se1.entrega_ubicacion();
            ubicacion_2 = se2.entrega_ubicacion();
            TipoTrafos = data.TransformadoresVU;
            trafos_comparar = TipoTrafos(:,3:4);
            indices = ismember(trafos_comparar, voltaje_trafos, 'rows');
...
function genera_proyectos_expansion(elementos_generados, pAdmProy, pParOpt)
    ...
    pAdmProy.inserta_matriz_estados_decision_primaria(estados_primarios.MatrizEstados);
    pAdmProy.inserta_capacidad_por_estado_decisiones_primarias(estados_primarios.CapacidadPorEstado);
    pAdmProy.inserta_estados_iniciales_decision_primaria(estados_primarios.EstadosIniciales);
    ...
    for bpar = baterias_existentes:nmax
        ...
        if bpar == 0
            ...
            for tipo_bat = 1:cant_tipos
                ...
                pAdmProy.agrega_proyecto_primario(proy, id_decision);
                proy_excluyentes(end+1) = proy;
...
%                end
                ...
                n_lineas_cs = length(LineasCS(id_corr).Conductor(id_cond).Compensacion(comp).Linea);
                if lpar <= n_lineas_cs
            ...
            for i = 1:ne_cs
                ...
                n_lineas_cs = length(LineasCS(id_corr).Conductor(id_cond_inicial).Compensacion(comp_inicial).Linea);
                if lpar + 1 <= n_lineas_cs
    ...
    for tpar = trafos_existentes:nmax
        ...
        if tpar == 0
            ...
            if tpar + 1 <= nmax
                ...
                MatrizEstados(id_decision).Estado(tpar,id_estado_final).IdProyectosSalientes = [MatrizEstados(id_decision).Estado(tpar,id_estado_final).IdProyectosSalientes proy.entrega_indice()];
...
function se_vur = crea_nueva_subestacion(se_base, voltaje, costo, pAdmProy)
    ...
    id_se = pAdmProy.entrega_cantidad_subestaciones()+1;
    se_vur = cSubestacion();
