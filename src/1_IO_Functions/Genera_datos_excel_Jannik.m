...
function data = importa_generadores(filename, data)%EL perfil se multiplca por la potencia máxima
    ...
    for fila = inicio:n
		...
		data.Generadores(contador_generadores, 4) = 0; %Q0, no toma en consideración 
		...
		data.Generadores(contador_generadores, 12) = num(fila,7)/10000; %costos generación
        data.Generadores(contador_generadores, 13) = num(fila,20); % tipo: 0: convencional, 1: eólico, 2: solar
...
function data = importa_lineas_transmision(filename, data)
    ...
    for fila = inicio:n
        ...
        bpu = num(fila, 7);
        sr = num(fila, 8);
        expansion = num(fila, 9);%¿Qué significa aquí el número de expansión? Ahora esta en 3
        largo = num(fila, 10);
        cinv = num(fila, 11); % en MM USD
        vnom = num(fila, 12);
        cantidad_vu = num(fila, 13);
        cantidad_cs = num(fila, 14);%¿Qué es esto?
        cantidad_cc = num(fila, 15);%¿Qué es esto?
...
function data = importa_baterias(filename, data)
    ...
    %la función any retorna un true si no hay zeros o l
    %la función strcmp sirve para comparar dos strings
    if sheetValid
...
function data = importa_tecnologias_bateria(filename, data)
    ...
    %Aquí compara si hay algúna hoja disponible llamada "storage tech" y
...
function data = importa_puntos_operacion(filename, data, id_punto_operacion)
    ...
    if tipo_po == 1
        while ~isnan(num(fila_po,col))
            ...
            % OJO AQUÍ, CAMBIE A +2 EL LADO DEL PESO
...
function data = importa_datos_embalse(filename, data)%Aquí metemos los datos de los embalses
    ...
    for fila = inicio:n
		...
		data.Embalses(contador_generadores, 2) = Vol_max*1000000; %Volumen máximo
		data.Embalses(contador_generadores, 3) = num(fila,4)*1000000; %Volumen mínimo
        ...
        data.Embalses(contador_generadores, 11) = num(fila,12); %Porcentaje filtración
    end
end

function data = importa_datos_hidro(filename, data)%Aquí metemos los datos de las centrales hidro
    ...
    for fila = inicio:n
		...
		data.Hidro(contador_generadores, 5) = num(fila,5); % % de filtración
		...
		data.Hidro(contador_generadores, 8) = num(fila,8); %Potencia máxima turbina
		data.Hidro(contador_generadores, 9) = num(fila,9); %Potencia mínima de turbina
	    data.Hidro(contador_generadores, 10) = num(fila,10)*1000000; %Flujo mínimo de salida
        data.Hidro(contador_generadores, 11) = num(fila,11); %Flujo máximo de salida
        data.Hidro(contador_generadores, 12) = num(fila,12)*1000000; %Vol. Max
        data.Hidro(contador_generadores, 13) = num(fila,13)*1000000; %Vol. Mín
