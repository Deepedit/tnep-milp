function importa_problema_optimizacion_tnep_garver(data, sep, adm_proy, pAdmSc, pParOpt)
    NivelDebug = 2;
    ...
    genera_proyectos_expansion(LineasBase, LineasCC, LineasCS, LineasVU, TrafosVU, adm_proy, pParOpt.ConsideraTransicionEstados);
...
function [LineasBase, LineasCC, LineasCS, LineasVU, TrafosVU] = genera_lineas(data, ...
    ...
    % LineasCC(id_corr).Existe = 0(no hay), 1(si hay)
    %
    ...
    for id_corr = 1:nc
        ...
        if largo == 0
            % transformadores base se ven después
            continue;
        end
        ...
        if param.considera_compensacion_serie() && largo > 80
            ...
            for id_linea = 1:length(LineasBase(id_corr).Linea)
                for comp = 1:ncomp_serie
                    ...
                    linea.Existente = false;
                    linea.Texto = ['NCS' num2str(comp) '_' num2str(lpar)];
...
function genera_proyectos_expansion(LineasBase, LineasCC, LineasCS, LineasVU, TrafosVU, adm_proy, considera_transicion_estados)
    genera_proyectos_expansion_subestaciones_transformadores(TrafosVU, adm_proy);
    genera_proyectos_expansion_lineas(LineasBase, LineasCC, LineasCS, LineasVU, adm_proy, considera_transicion_estados);
...
function genera_proyectos_expansion_lineas(LineasBase, LineasCC, LineasCS, LineasVU, adm_proy, considera_transicion_estados)
        ...
        for lpar = lineas_existentes:nmax
            ...
            if lpar == 0
                ...
                for i = 1:ne_vu
                    ...
                    proy_conectividad_trafos_se1 = adm_proy.entrega_proyectos_transformadores(linea.entrega_se1(), 1); %1 es índice paralelo
                ...
                for i = 1:ne_vu
                    ...
                    proy_conectividad_trafos_se1 = adm_proy.entrega_proyectos_transformadores(lineas_a_agregar(1).entrega_se1(), 1); %1 es índice paralelo
                ...
                for i = 1:ne_cc
                    ...
                    if lpar <= n_lineas_cc                    
                        ...
                        if considera_transicion_estados
                            for j = 1:ne_vu
                                id_cond_vu = EstadosVU(j,1);
                                vur = EstadosVU(j,2);
                                n_lineas_vu = length(LineasVU(id_corr).VUR(vur).Conductor(id_cond_vu).Linea);
                                if lpar <= n_lineas_vu                
                                    lineas_a_agregar = LineasVU(id_corr).VUR(vur).Conductor(id_cond_vu).Linea(1:lpar);
                                    proy = crea_proyecto_cambio_estado(lineas_a_remover, lineas_a_agregar, 'AV');
                                    proy.EsUprating = true;
                                    if ~isempty(EstadosConducentes.CC(i).Proyectos)
                                        proy.TieneDependencia = true;
...
function proy = crea_proyecto_cambio_estado(lineas_a_remover, lineas_a_agregar, tipo_proyecto)
    ...
    elseif strcmp(tipo_proyecto,'AV') 
        ...
        costo_linea_nueva = (lineas_a_agregar(1).entrega_costo_conductor()+lineas_a_agregar(1).entrega_costo_torre());
        costo_linea_nueva = costo_linea_nueva*length(lineas_a_agregar);
        ...
        if tipo_cond_inicial == tipo_cond_final
            % VU se realiza con el mismo conductor. Los costos son el 20%
            % de lo que hubiera salido
            proy.inserta_cambio_conductor_aumento_voltaje(false);
            proy.inserta_costo_inversion(0.2*costo_linea_nueva);
        else
            % costo de VU es lo mismo que construir línea nueva. 
            proy.inserta_cambio_conductor_aumento_voltaje(true);
            costo_inversion = lineas_a_agregar(1).entrega_costo_conductor()+lineas_a_agregar(1).entrega_costo_torre()+lineas_a_agregar(1).entrega_costo_servidumbre();
            costo_inversion = costo_inversion*length(lineas_a_agregar);
            proy.inserta_costo_inversion(costo_inversion);
        end
    else
...
% RAMRAM TODO: esta función no se utiliza más, ya que las restricciones de
% conectividad están todas agrupadas en los proyectos de VU
	for i= 1:length(adm_proy.Proyectos)
        if strcmp(adm_proy.Proyectos(i).entrega_tipo_proyecto(),'AS')
            ...
            for j = 1:length(adm_proy.Proyectos)
                if strcmp(adm_proy.Proyectos(j).entrega_tipo_proyecto(),'AV')
                    for ii = 1:length(adm_proy.Proyectos(j).Elemento)
                        if strcmp(adm_proy.Proyectos(j).Accion{ii},'A')
                            el_red = adm_proy.Proyectos(j).Elemento(ii);
                ...
                elseif strcmp(adm_proy.Proyectos(j).entrega_tipo_proyecto(),'AL')
                    el_red = adm_proy.Proyectos(j).Elemento(end);
                    if (el_red.entrega_indice_paralelo() == 1) && (el_red.entrega_se1() == se || el_red.entrega_se2() == se)
