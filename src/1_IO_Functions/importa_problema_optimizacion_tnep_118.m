...
function [ElementosBase, LineasCC, LineasCS, LineasVU, TrafosVU] = genera_elementos_red(data, ...
    ...
    % agrega id de líneas y trafos proyectados y nuevas subestaciones
...
%         else
%             error = MException('cimporta_problema_optimizacion_tnep:genera_elementos_red','tipo elemento para expansión aún no implementado');
%             throw(error)
%         end
%     end
...
function [ElementosBase, LineasCC, LineasCS, LineasVU, TrafosVU] = genera_lineas(ElementosBase, LineasCC, LineasCS, LineasVU, TrafosVU, id_corr, data, param, sep, adm_proy)
    ...
    for aux = 1:naux
        if LineasAux(aux, 9) ~= conductor_base_primero
        	...
        	texto = 'Tipo de conductor para línea paralela no coincide';
    ...
    if nmax > nlineas_existentes && distintos_conductores_base
        % por ahora no se permite crear líneas nuevas cuando los
        % conductores base no coinciden
        texto = ['Error en datos de entrada. En id_corr ' num2str(id_corr) ' el número de líneas es mayor al existente, pero hay más de un conductor tipo. No se ha visto este caso aún'];
        error = MException('importa_problema_optimizacion_tnep:genera_lineas',texto);
        throw(error)
    end
    ...
    if param.considera_voltage_uprating() && ~isempty(voltajes_vu) && nlineas_existentes > 0 && nmax > nlineas_existentes
        ...
        if ~param.cambio_conductor_voltage_uprating()
            ...
            if length(unique(conductor_base)) > 1
                texto = ['Error en datos de entrada. En corredor ' num2str(id_corr) ' hay más de un conductor base. No se puede hacer voltage uprating por ahora'];
                %error = MException('importa_problema_optimizacion_tnep:genera_lineas',texto);
                %throw(error)
                warning(texto)
            end
    ...
    for lpar = 1:nmax
        ...
        if lpar <= nlineas_existentes
            % línea existente. Se crea la línea con el conductor base
            ...
            % línea no existente. Se crea en base a conductor "base"
