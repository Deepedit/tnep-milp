...
%VarOperacion = cElementoRed.empty
        ...
        TipoVarOpt = [] % 1: expansión variables binarias, 2: operación y expansion variables continuas (2nd-level)
        % Subtipo varopt: 
        % Tipo 1. (expansion variables binarias): 1: decisión expansion, 2: var. auxiliar (incluye costos de inversión), 3) inv. acumulada. Se utiliza en operación también; 
        % Tipo 2: 1: expansión de la generacion, 2: UC 3: operación
        SubtipoVarOpt = [] 
        EscenarioVarOpt = [] % 0: todos, i: escenario i
        EtapaVarOpt = [] % nro. etapa
        TipoRestriccionIneq = []; % 1: expansión variables binarias, 2: operación y expansión variables continuas (2nd-level)
        TipoRestriccionEq = []; % 1: expansión variables binarias, 2: operación y expansión variables continuas (2nd-level)
    ...
    methods
        ...
        function this = cOptMILPGxTx(sep, adm_escenarios, adm_proy, par_opt)
			this.pSEP = sep;
            ...
            this.iCantEtapas = 0;
            this.iCantPuntosOperacion = 0;  
            this.iIndiceIneq = 0;
            this.iIndiceEq = 0;
            this.iNivelDebug = par_opt.NivelDebug;
            % uncertainties
            this.iCantEscenarios = par_opt.entrega_cantidad_escenarios;
        ...
        function inicializa_variables_decision(this)
            ...
            this.VarOperacion = this.pSEP.entrega_generadores();
            ...
            % elementos de decision de expansion de la transmisión
            ...
            this.VarOperacion = [this.VarOperacion; this.pAdmProy.entrega_generadores_expansion()];
        ...
        function inicializa_variables_decision_expansion(this, escenario)
            ...
            for i=1:length(this.VarExpansion)
                indice_exp_desde = this.iCantVarDecision + 1;
                indice_exp_hasta = this.iCantVarDecision + this.iCantEtapas;
                this.iCantVarDecision = indice_exp_hasta;
                
                proy = this.VarExpansion(i);
                proy.inserta_varopt_expansion('Decision', escenario, indice_exp_desde);
                
                this.lb(indice_exp_desde:indice_exp_hasta) = 0;
                if proy.entrega_tipo_decision() == 1
                    % variable discreta
                    ...
                    % variable continua
                    this.ub(indice_exp_desde:indice_exp_hasta) = round(proy.entrega_pmax()/sbase,dec_redondeo);
                    this.TipoVarOpt(indice_exp_desde:indice_exp_hasta) = 2;
                    this.SubtipoVarOpt(indice_exp_desde:indice_exp_hasta) = 1;                
                end
                
                this.EscenarioVarOpt(indice_exp_desde:indice_exp_hasta) = escenario;
                this.EtapaVarOpt(indice_exp_desde:indice_exp_hasta) = 1:1:this.iCantEtapas;
                
                if proy.entrega_tipo_decision() == 1
                    % variable acumulada. Para decisiones binarias
                    indice_cum_desde = this.iCantVarDecision + 1;
                    indice_cum_hasta = this.iCantVarDecision + this.iCantEtapas;
                    this.iCantVarDecision = indice_cum_hasta;
                    proy = this.VarExpansion(i);
                    proy.inserta_varopt_expansion('Acumulada', escenario, indice_cum_desde);
                    ...
                    this.EtapaVarOpt(indice_cum_desde:indice_cum_hasta) = 1:1:this.iCantEtapas;

                    if this.pParOpt.considera_valor_residual_elementos()
                        % costos asociados a variables de expansión binarias en caso de considerar valor residual elementos (uprating)
                        indice_costo_desde = this.iCantVarDecision+1;
                        indice_costo_hasta = this.iCantVarDecision + this.iCantEtapas ;
                        this.iCantVarDecision = indice_costo_hasta; % último valor actual de CantVarDecision

                        proy.inserta_varopt_expansion('Costo', escenario, indice_costo_desde);
                        costo_max = proy.entrega_costos_inversion();
                        ...
                        this.EtapaVarOpt(indice_costo_desde:indice_costo_hasta) = 1:1:this.iCantEtapas;
                        %this.ub(this.iCantVarDecision) = inf;
                    end
                elseif proy.entrega_cantidad_decisiones() == 2
                    % proyecto de decisión continua con más de una variable de decisión (pueden ser baterías continuas)
                    indice_exp2_desde = this.iCantVarDecision + 1;
                    indice_exp2_hasta = this.iCantVarDecision + this.iCantEtapas;
                    this.iCantVarDecision = indice_exp2_hasta;

                    proy.inserta_varopt_expansion('Decision2', escenario, indice_exp2_desde);

                    this.lb(indice_exp2_desde:indice_exp2_hasta) = 0;
                    this.ub(indice_exp2_desde:indice_exp2_hasta) = round(proy.entrega_emax()/sbase,dec_redondeo);
                    this.TipoVarOpt(indice_exp2_desde:indice_exp2_hasta) = 2;
                    this.SubtipoVarOpt(indice_exp2_desde:indice_exp2_hasta) = 1;
                end

                if this.iNivelDebug > 1
                    %for etapa = 1:this.iCantEtapas
                    nombre = this.entrega_nombre_proyecto_expansion(proy,1, i);
                    ...
                    elseif proy.entrega_cantidad_decisiones() == 2
                        nombre = this.entrega_nombre_proyecto_expansion(proy,2, i);
                        indice_exp2 = (indice_exp2_desde:1:indice_exp2_hasta);
                        this.NombreVariables(indice_exp2) = cellstr(nombre);
                    end
                end
            end
        end
...
%            this.Aineq = sparse(cantidad_ineq, this.iCantVarDecision);
        ...
        function escribe_restricciones_proyectos(this)
            ...
            for proy = 1:cant_proyectos
                ...
                for escenario = 1:this.iCantEscenarios
                    ...
                    for etapa = 1:this.iCantEtapas
                        ...
                        %this.Aeq(this.iIndiceEq,indice_desde:indice_hasta) = -1;
                        this.agrega_valores_matriz_igualdad(etapa,this.iIndiceEq, indice_desde:indice_hasta,-1);
                                                
                        %this.Aeq(this.iIndiceEq,indice_acumulado) = 1;
            ...
            for i = 1:cant_proy_conectividad
                ...
                proy_con = this.pAdmProy.ProyTransmision(i);
                for escenario = 1:this.iCantEscenarios
            ...
            for i = 1:length(el_red)
                if isa(el_red, 'cBateria')
                    ...
                    for periodo = 1:cant_periodos_representativos
                        ...
                        this.agrega_valores_matriz_igualdad(ancho_periodo,indice_eq_desde:indice_eq_hasta,indice_e_desde_periodo:indice_e_hasta_periodo,1); % E(t)
                        this.agrega_valores_matriz_igualdad(ancho_periodo-1,indice_eq_desde+1:indice_eq_hasta,indice_e_desde_periodo:indice_e_hasta_periodo-1,-1); % -E(t-1)
                        this.agrega_valores_matriz_igualdad(ancho_periodo-1,indice_eq_desde+1:indice_eq_hasta,indice_p_descarga_desde_periodo:indice_p_descarga_hasta_periodo-1,round(1/eta_descarga, dec_redondeo)); % +Pdescarga(t-1)
                        this.agrega_valores_matriz_igualdad(ancho_periodo-1,indice_eq_desde+1:indice_eq_hasta,indice_p_carga_desde_periodo:indice_p_carga_hasta_periodo-1,round(-eta_carga, dec_redondeo)); % -Pcarga(t-1)
                        this.agrega_valores_matriz_igualdad(1,indice_eq_desde,indice_e_hasta_periodo,-1); % -E(t_fin)
...
% end
                ...
                big_m_sistema = round(par_disy/reactancia,dec_redondeo);
                big_m_vu = round(this.entrega_parametro_disyuntivo_vu(se_1.entrega_id(), se_2.entrega_id())/reactancia,dec_redondeo);
                big_m_cscc = round(this.entrega_parametro_disyuntivo_cscc(se_1.entrega_id(), se_2.entrega_id())/reactancia,dec_redondeo);
                big_m_conv = round(this.entrega_parametro_disyuntivo_conv(se_1.entrega_id(), se_2.entrega_id())/reactancia,dec_redondeo);
		...
		function optimiza(this)
            ...
            elseif strcmp(this.pParOpt.EstrategiaOptimizacion,'Benders')
                if this.pParOpt.ComputoParalelo
                    this.calcula_costos_operacion_sin_restricciones_paralelo();
                else
                    this.calcula_costos_operacion_sin_restricciones_secuencial();
                end
                estrategia = 1;
                if estrategia == 1
                    if this.pParOpt.ComputoParalelo
...
%disp('Verificar que variables de decisión de la expansión estén en bloques')
            ...
            if this.iNivelDebug > 2
                ...
                for escenario = 1:this.iCantEscenarios
                    for etapa = 1:this.iCantEtapas
                        ...
                        nombres_alpha{pos_alpha} = ['Alpha_S' num2str(escenario) '_E' num2str(etapa)];
                    end
                end
            ...
            for escenario = 1:this.iCantEscenarios
                peso_escenario = this.pAdmSc.entrega_peso_escenario(escenario);
                pesos_alpha = [pesos_alpha round(peso_escenario*descuento_etapas,dec_redondeo)];
            end
            ...
            for escenario = 1:this.iCantEscenarios
                for etapa = 1:this.iCantEtapas
                    pos_alpha = etapa + this.iCantEtapas*(escenario -1);
                    lb_alphas(pos_alpha) = this.COperUninodal(escenario, etapa)/pesos_alpha(pos_alpha);
                end
            end
                ...
                for oper = 1:this.iCantPuntosOperacion
                    ...
                    for se = 1:length(subestaciones)
                        ...
                        for gen = 1:length(generadores_res)
                            ...
                            perfil_ernc = this.pAdmSc.entrega_perfil_ernc(indice_perfil);
                            if generadores_res(gen).EvolucionCapacidadAFuturo(escenario)
...
%                            if se_1 ~= this.pSEP.Subestaciones(se)
        ...
        function calcula_parametros_disyuntivos(this)
            ...
            [id_proy_elegibles, ~] = this.pAdmProy.determina_espacio_busqueda(plan_vacio);
            ...
            for corr = 1:cant_corredores
                ...
                if corredores_a_calcular(corr,3) == 0
                    ...
                    for z_corr = 0:length(id_proyectos_corredor)
                        ...
                        if z_corr > 0
                            [id_proy_elegibles_caso_actual, ~] = this.pAdmProy.determina_espacio_busqueda(plan_actual);
                        else
...
%            fprintf(this.docIDFico, ['results: array(1..' num2str(this.iCantVarDecision) ') of real\n']);
        ...
        function calcula_costos_operacion_sin_restricciones_secuencial(this)
            disp('Calcula costos de operacion sin restricciones modo secuencial')
            this.COperUninodal = zeros(this.iCantEscenarios, this.iCantEtapas);
            ...
            for escenario = 1:this.iCantEscenarios
                for etapa = 1:this.iCantEtapas
                    ...
                    [Aeq_oper_sr, beq_actual] = this.entrega_rest_be_uninodal_por_escenario_etapa(escenario, etapa);
                    ...
                    elseif strcmp(this.pParOpt.Solver, 'Xpress')
                        ...
                        disp(['Tiempo en resolver operación sin restricciones escenario ' num2str(escenario) ' etapa ' num2str(etapa) ': ' num2str(toc)])
                        this.COperUninodal(escenario, etapa) = fval;
                    end
                end
            end
        end

        function calcula_costos_operacion_sin_restricciones_paralelo(this)
            ...
            coper = zeros(this.iCantEscenarios*this.iCantEtapas,1);
            parfor j = 1:this.iCantEscenarios*this.iCantEtapas
                t_inicio = clock;
                [etapa,escenario]=ind2sub([this.iCantEtapas, this.iCantEscenarios],j);
                ...
                disp(['Tiempo en resolver operación sin restricciones escenario ' num2str(escenario) ' etapa ' num2str(etapa) ':' num2str(tiempo)])
            end
            
            for escenario = 1:this.iCantEscenarios
                for etapa = 1:this.iCantEtapas
                    posicion = etapa + this.iCantEtapas*(escenario -1);
                    this.COperUninodal(escenario, etapa) = coper(posicion);
                end
            end
        end
        
        function [Aeq_oper_sr, beq_oper_sr] = entrega_rest_be_uninodal_por_escenario_etapa(this, escenario, etapa)
            % dimensiones
            [~,cant_variables_decision] = size(this.Aeq);
            cant_po = this.iCantPuntosOperacion;            
            Aeq_oper_sr = zeros(cant_po, cant_variables_decision);
                ...
                if strcmp(proy.entrega_tipo_proyecto(), 'AV')
                    ...
                    for no_grupo = 1:cantidad_grupos_conectividad
                        ...
                        elseif proy_con(1).Elemento(1).entrega_se2().Existente
                            ...
                            error = MException('cOptMILP:calcula_parametros_disyuntivos',...
                        ...
                        elseif isempty(se_2_red_orig)
                            ...
                            error = MException('cOptMILP:calcula_parametros_disyuntivos','Todas las subestaciones existentes fueron identificadas. Error de programacion');
                            throw(error)                            
                        end
                    end
                ...
                elseif strcmp(proy.entrega_tipo_proyecto(), 'CC') || ...
                   strcmp(proy.entrega_tipo_proyecto(), 'CS')
                    if strcmp(proy.Accion(end),'A') ~= 1
                        error = MException('cOptMILP:calcula_parametros_disyuntivos','Accion final de proyecto de cambio de estado no es agrega elemento');
                        throw(error)
                    end                        
        ...
        function nombre = entrega_nombre_proyecto_expansion(this, proy, decision, id_var_expansion)
            etapa = (1:1:this.iCantEtapas)';
            ...
            elseif strcmp(tipo_proyecto, 'AB') && proy.entrega_tipo_decision() == 1
                % baterías discretas (i.e. potencia y capacidad están fijas)
                ...
                nombre = strcat('Pr_', num2str(id_var_expansion), '_AB_', num2str(id_par), '_T', num2str(tipo_bat), '_V', num2str(vn), ...
                          '_B', num2str(id_ubic), '_E', num2str(etapa, '%02d'), '_S', num2str(escenario));
            elseif strcmp(tipo_proyecto, 'AB') && proy.entrega_tipo_decision() == 2
                % baterías continuas (i.e. potencia y capacidad son decisión de optimización)
                id_ubic = proy.Elemento(1).entrega_se().entrega_id();
                tipo_bat = proy.Elemento(1).entrega_tipo_bateria();
                vn = proy.Elemento(1).entrega_se().entrega_vn();
                if decision == 1
                    nombre = strcat('Pr_', num2str(id_var_expansion), '_AB_P', '_T', num2str(tipo_bat), '_V', num2str(vn), ...
                              '_B', num2str(id_ubic), '_E', num2str(etapa, '%02d'), '_S', num2str(escenario));
                else
                    nombre = strcat('Pr_', num2str(id_var_expansion), '_AB_E', '_T', num2str(tipo_bat), '_V', num2str(vn), ...
                              '_B', num2str(id_ubic), '_E', num2str(etapa, '%02d'), '_S', num2str(escenario));
                end
            elseif strcmp(tipo_proyecto, 'ACSP') && proy.entrega_tipo_decision() == 1
                % Planta CSP disreta
                id_ubic = proy.Elemento(1).entrega_se().entrega_id();
                id_par = proy.Elemento(1).entrega_indice_paralelo();
                tipo_tecnologia = proy.Elemento(1).entrega_tipo_tecnologia();
                vn = proy.Elemento(1).entrega_se().entrega_vn();
                nombre = strcat('Pr_', num2str(id_var_expansion), '_ACSP_', num2str(id_par), '_T', num2str(tipo_tecnologia), '_V', num2str(vn), ...
                          '_B', num2str(id_ubic), '_E', num2str(etapa, '%02d'), '_S', num2str(escenario));
            elseif strcmp(tipo_proyecto, 'ACSP') && proy.entrega_tipo_decision() == 2
                % Planta CSP disreta
                id_ubic = proy.Elemento(1).entrega_se().entrega_id();
                id_par = proy.Elemento(1).entrega_indice_paralelo();
                tipo_tecnologia = proy.Elemento(1).entrega_tipo_tecnologia();
                vn = proy.Elemento(1).entrega_se().entrega_vn();
                if decision == 1
                    nombre = strcat('Pr_', num2str(id_var_expansion), '_ACSP_P', num2str(id_par), '_T', num2str(tipo_tecnologia), '_V', num2str(vn), ...
                              '_B', num2str(id_ubic), '_E', num2str(etapa, '%02d'), '_S', num2str(escenario));
                else
                    nombre = strcat('Pr_', num2str(id_var_expansion), '_ACSP_E', '_T', num2str(tipo_tecnologia), '_V', num2str(vn), ...
                              '_B', num2str(id_ubic), '_E', num2str(etapa, '%02d'), '_S', num2str(escenario));
                end
            elseif strcmp(tipo_proyecto, 'AG') && proy.entrega_tipo_decision() == 1
                % Generador discreto
                id_ubic = proy.Elemento(1).entrega_se().entrega_id();
                id_par = proy.Elemento(1).entrega_indice_paralelo();
                tipo_tecnologia = proy.Elemento(1).entrega_tipo_tecnologia();
                vn = proy.Elemento(1).entrega_se().entrega_vn();
                if proy.Elemento(1).entrega_tipo_central() == 1
                    texto_variable = '_AGConv_';
                else
                    texto_variable = '_AGERNC_';
                end
                nombre = strcat('Pr_', num2str(id_var_expansion), texto_variable, num2str(id_par), '_T', num2str(tipo_tecnologia), '_V', num2str(vn), ...
                          '_B', num2str(id_ubic), '_E', num2str(etapa, '%02d'), '_S', num2str(escenario));
            elseif strcmp(tipo_proyecto, 'AG') && proy.entrega_tipo_decision() == 2
                % Generador continuo
                id_ubic = proy.Elemento(1).entrega_se().entrega_id();
                %id_par = proy.Elemento(1).entrega_indice_paralelo();
                tipo_tecnologia = proy.Elemento(1).entrega_tipo_tecnologia();
                ...
                nombre = strcat('Pr_', num2str(id_var_expansion), texto_variable, '_T', num2str(tipo_tecnologia), '_V', num2str(vn), ...
                          '_B', num2str(id_ubic), '_E', num2str(etapa, '%02d'), '_S', num2str(escenario));
            end
        end
    end
