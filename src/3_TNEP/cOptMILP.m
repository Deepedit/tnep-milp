...
%VarOperacion = cElementoRed.empty
    ...
    methods
        ...
        function this = cOptMILP(sep, adm_escenarios, adm_proy, par_opt)
			this.pSEP = sep;
        ...
        function escribe_problema_optimizacion(this)
            ...
            disp(['Tiempo en calcular parametros disyuntivos: ' num2str(etime(clock,t_inicio))])
            ...
            disp(['Tiempo crear matrices igualdad/desigualdad: ' num2str(etime(clock,t_inicio))])
        ...
        function inicializa_variables_decision_expansion(this, escenario)
            ...
            for i=1:length(this.VarExpansion)
                ...
                indice_desde_exp = this.iCantVarDecision;
                indice_hasta_exp = indice_desde_exp + this.iCantEtapas - 1;
                this.iCantVarDecision = indice_hasta_exp;
                
                this.VarExpansion(i).inserta_varopt_expansion('Decision', escenario, indice_desde_exp);
                
                this.intcon = [this.intcon indice_desde_exp:1:indice_hasta_exp];
                this.lb(indice_desde_exp:indice_hasta_exp) = 0;
                this.ub(indice_desde_exp:indice_hasta_exp) = 1;
                this.TipoVarOpt(indice_desde_exp:indice_hasta_exp) = 1;
                this.SubtipoVarOpt(indice_desde_exp:indice_hasta_exp) = 1;
                
                this.EscenarioVarOpt(indice_desde_exp:indice_hasta_exp) = escenario;
                this.EtapaVarOpt(indice_desde_exp:indice_hasta_exp) = 1:1:this.iCantEtapas;
                this.SubetapaVarOpt(indice_desde_exp:indice_hasta_exp) = 0;
...
%end
                ...
                % variable acumulada
                this.iCantVarDecision = this.iCantVarDecision + 1;
                indice_desde_cum = this.iCantVarDecision;
                indice_hasta_cum = indice_desde_cum + this.iCantEtapas - 1;
                this.iCantVarDecision = indice_hasta_cum;
                this.VarExpansion(i).inserta_varopt_expansion('Acumulada', escenario, indice_desde_cum);
                
                this.intcon = [this.intcon indice_desde_cum:1:indice_hasta_cum];
                this.lb(indice_desde_cum:indice_hasta_cum) = 0;
                this.ub(indice_desde_cum:indice_hasta_cum) = 1;
                this.TipoVarOpt(indice_desde_cum:indice_hasta_cum) = 1;
                this.SubtipoVarOpt(indice_desde_cum:indice_hasta_cum) = 3; % involucra tanto decisión de expansión como operación
                
                this.EscenarioVarOpt(indice_desde_cum:indice_hasta_cum) = escenario;
                this.EtapaVarOpt(indice_desde_cum:indice_hasta_cum) = 1:1:this.iCantEtapas;
                this.SubetapaVarOpt(indice_desde_cum:indice_hasta_cum) = 0;

                if this.pParOpt.considera_valor_residual_elementos()
                    indice_desde_costo = this.iCantVarDecision+1;
                    indice_hasta_costo = indice_desde_costo + this.iCantEtapas - 1;

                    this.iCantVarDecision = indice_hasta_costo; % último valor actual de CantVarDecision
                
                    this.VarExpansion(i).inserta_varopt_expansion('Costo', escenario, indice_desde_costo);
                    ...
                    this.lb(indice_desde_costo:indice_hasta_costo) = 0;
                    this.ub(indice_desde_costo:indice_hasta_costo) = round(costo_max,dec_redondeo); % para estabilidad numérica
                    this.TipoVarOpt(indice_desde_costo:indice_hasta_costo) = 1;
                    this.SubtipoVarOpt(indice_desde_costo:indice_hasta_costo) = 2;
                    this.EscenarioVarOpt(indice_desde_costo:indice_hasta_costo) = escenario;
                    this.EtapaVarOpt(indice_desde_costo:indice_hasta_costo) = 1:1:this.iCantEtapas;
                    this.SubetapaVarOpt(indice_desde_costo:indice_hasta_costo) = 0;
                    
                    %this.ub(this.iCantVarDecision) = inf;
                end

                if this.iNivelDebug > 1
                    %for etapa = 1:this.iCantEtapas
                    nombre = this.entrega_nombre_proyecto_expansion(this.VarExpansion(i), escenario, i);
                    indice_exp = (indice_desde_exp:1:indice_desde_exp+this.iCantEtapas-1);
                    this.NombreVariables(indice_exp) = cellstr(nombre);

                    indice_cum = (indice_desde_cum:1:indice_desde_cum+this.iCantEtapas-1); 
                    this.NombreVariables(indice_cum) = cellstr(strcat('Cum_', nombre));
                    if this.pParOpt.considera_valor_residual_elementos()
                        indice_costo = (indice_desde_costo:1:indice_desde_costo+this.iCantEtapas-1)';
                            ...
                            if ~isempty(proyectos_dependientes)
                                for dep = 1:length(proyectos_dependientes)
                                    ...
                                    for etapa_previa = 1:etapa
                                        this.iCantVarDecision = this.iCantVarDecision + 1;
                                        indice_proy_dep = indice_proy_dep_desde + etapa_previa - 1;
            ...
            for i = 1:length(this.VarExpansion)
                for escenario = 1:this.iCantEscenarios
                    ...
                    if this.pParOpt.considera_valor_residual_elementos()
                        ...
                        this.Fobj(indice_costo_desde:indice_costo_hasta) = round(peso_escenario*descuento_etapas,dec_redondeo);
                    else
                        indice_expansion_desde = this.VarExpansion(i).entrega_varopt_expansion('Decision', escenario);
                        indice_expansion_hasta = indice_expansion_desde + this.iCantEtapas - 1;
                        ...
                        valor = round(costo_inversion*peso_escenario*descuento_etapas,dec_redondeo);
...
disp('Verificar nuevas ecuaciones para calculo de decision de expansion acumulada')
            contador = 0;
            ...
            for proy = 1:cant_proyectos
                ...
                for escenario = 1:this.iCantEscenarios
                    ...
                    for etapa = 1:this.iCantEtapas
                        ...
                        indice_acumulado = indice_desde_acumulado + etapa - 1;
%                        indice_hasta = indice_desde + etapa - 1;
%                        this.agrega_valores_matriz_igualdad(etapa,this.iIndiceEq, indice_desde:indice_hasta,-1);                                                
%                        this.agrega_valores_matriz_igualdad(1,this.iIndiceEq, indice_acumulado,1);
indice_actual = indice_desde + etapa - 1;
                        %acumulado(t) = decision(t) + acumulado(t-1)
this.agrega_valores_matriz_igualdad(1,this.iIndiceEq, indice_acumulado,1);
this.agrega_valores_matriz_igualdad(1,this.iIndiceEq, indice_actual,-1);
if etapa > 1
	this.agrega_valores_matriz_igualdad(1,this.iIndiceEq, indice_acumulado-1,-1);
end                        
                    ...
                    if this.pAdmProy.ProyTransmision(proy).TieneDependencia
                        for etapa = 1:this.iCantEtapas
                            ...
                            indice_proy = indice_desde_acumulado + etapa - 1;
                            ...
                            for dep = 1:length(dependencias)
                                %indice_dependencia_desde = dependencias(dep).entrega_varopt_expansion('Decision',escenario);
                                %indice_dependencia_hasta = indice_dependencia_desde + etapa - 1;
                                %this.Aineq(this.iIndiceIneq,indice_dependencia_desde:indice_dependencia_hasta) = -1;
                                indice_dependencia = dependencias(dep).entrega_varopt_expansion('Acumulada',escenario) + etapa - 1;
                                %this.Aineq(this.iIndiceIneq,indice_dependencia) = -1;
                        ...
                        for etapa = 1:this.iCantEtapas
                            ...
                            for no_grupo = 1:cantidad
                                ...
                                for ii = 1:length(proy_con)
                                    ...
                                    indice_con = proy_con(ii).entrega_varopt_expansion('Acumulada',escenario) + etapa - 1;
                ...
                % restricciones por proyecto entre escenarios
                % proyectos se debe construir en todos los escenarios para short-term 
                etapas_para_entrada_en_operacion = this.pAdmProy.ProyTransmision(proy).entrega_etapas_entrada_en_operacion();
                for etapa = 1:etapas_para_entrada_en_operacion
                    % TODO: Test para ver qué opción es mejor: más
                    % restricciones o menos
                    if this.pParOpt.TestRestriccionesProyEscenariosExhaustivas
                        for escenario_1 = 1:this.iCantEscenarios
                            for escenario_2 = escenario_1 + 1:this.iCantEscenarios
                                this.iIndiceEq = this.iIndiceEq + 1;
                                indice_proy_escenario_1 = indice_base_proy_por_escenario(escenario_1) + etapa - 1;
                                indice_proy_escenario_2 = indice_base_proy_por_escenario(escenario_2) + etapa - 1;

                                %this.Aeq(this.iIndiceEq,indice_proy_escenario_1) = 1;
                                this.agrega_valores_matriz_igualdad(1,this.iIndiceEq, indice_proy_escenario_1,1);
                                %this.Aeq(this.iIndiceEq,indice_proy_escenario_2) = -1;
                                this.agrega_valores_matriz_igualdad(1,this.iIndiceEq, indice_proy_escenario_2,-1);
                                ...
                                if this.iNivelDebug > 1
                                    proy_id = this.pAdmProy.ProyTransmision(proy).entrega_indice();
                                    this.NombreEq{this.iIndiceEq} = strcat('req_', num2str(this.iIndiceEq), ...
                                        '_delay_entrada_', 'P', num2str(proy_id), '_E1_', num2str(escenario_1), '_E2_', num2str(escenario_2), '_E', num2str(etapa));
                                end
                                
                            end
                        end
                    else
                        % menos restricciones (sólo las necesarias)
                        for escenario_1 = 1:this.iCantEscenarios-1
                            this.iIndiceEq = this.iIndiceEq + 1;
                            indice_proy_escenario_1 = indice_base_proy_por_escenario(escenario_1) + etapa - 1;
                            indice_proy_escenario_2 = indice_base_proy_por_escenario(escenario_1+1) + etapa - 1;
                            %this.Aeq(this.iIndiceEq,indice_proy_escenario_1) = 1;
                            this.agrega_valores_matriz_igualdad(1,this.iIndiceEq, indice_proy_escenario_1,1);
                            %this.Aeq(this.iIndiceEq,indice_proy_escenario_2) = -1;
                            this.agrega_valores_matriz_igualdad(1,this.iIndiceEq, indice_proy_escenario_2,-1);
                            ...
                            if this.iNivelDebug > 1
                                proy_id = this.pAdmProy.ProyTransmision(proy).entrega_indice();
                                this.NombreEq{this.iIndiceEq} = strcat('req_', num2str(this.iIndiceEq), ...
                                    '_delay_entrada_', 'P', num2str(proy_id), '_E1_', num2str(escenario_1), '_E2_', num2str(escenario_1+1), '_E', num2str(etapa));
                            end
                        end
                    end
                end
            ...
            for i = 1:cant_proy_conectividad
                ...
                for escenario = 1:this.iCantEscenarios
                    ...
                    indice_ineq_hasta = indice_ineq_desde + this.iCantEtapas - 1;
                    this.iIndiceIneq = indice_ineq_hasta;
                    
                    indice_proy_con_desde = proy_con.entrega_varopt_expansion('Acumulada',escenario);
                    indice_proy_con_hasta = indice_proy_con_desde + this.iCantEtapas - 1;
                    
                    %this.Aineq(indice_ineq_desde:indice_ineq_hasta,indice_proy_con_desde:indice_proy_con_hasta) = diag(ones(this.iCantEtapas,1));
                    this.agrega_valores_matriz_desigualdad(this.iCantEtapas,indice_ineq_desde:indice_ineq_hasta, indice_proy_con_desde:indice_proy_con_hasta,1);
                    ...
                    this.EtapaRestriccionIneq(indice_ineq_desde:indice_ineq_hasta) = 1:1:this.iCantEtapas;
                    ...
                    for j = 1:length(RelProyConectProyPrimarios(i).ProyPrimario)
                        ...
                        indice_proy_ppal_hasta = indice_proy_ppal_desde + this.iCantEtapas - 1;
                        ...
                        this.agrega_valores_matriz_desigualdad(this.iCantEtapas,indice_ineq_desde:indice_ineq_hasta, indice_proy_ppal_desde:indice_proy_ppal_hasta,-1);
                        
                    end
            
                    if this.iNivelDebug > 1
                        etapa = (1:1:this.iCantEtapas)';
                        indices = (indice_ineq_desde:1:indice_ineq_desde+this.iCantEtapas-1)';
...
%                         for etapa = 1:this.iCantEtapas
%                             this.NombreIneq{indice_ineq_desde + etapa - 1} = strcat('rineq_', num2str(indice_ineq_desde + etapa - 1), ...
%                                 '_lim_inf_proy_con_', 'P', num2str(id_proy_con), '_S', num2str(escenario), '_E', num2str(etapa));
%                         end
                    end
                end
            end
            ...
            for i = 1:length(this.pAdmProy.ProyTransmisionObligatorios)
                ...
                for escenario = 1:this.iCantEscenarios
                    ...
                    for j = 1:length(this.pAdmProy.ProyTransmisionObligatorios(i).Proyectos)
                        indice_proy = this.pAdmProy.ProyTransmisionObligatorios(i).Proyectos(j).entrega_varopt_expansion('Decision', escenario);
                    ...
                    if this.iNivelDebug > 1
                        this.NombreIneq{this.iIndiceIneq} = strcat('rineq_', num2str(this.iIndiceIneq), '_oblig_exp_P', num2str(proy_id), '_E', num2str(1), '_S', num2str(escenario)); % solo para la primera etapa
                    end
                end
            end
            ...
            for indice = 1:length(this.pAdmProy.ProyTransmisionExcluyentes)
                ...
                for escenario = 1:this.iCantEscenarios
                    test_rest_exluyentes_por_etapas = false;
                    if test_rest_exluyentes_por_etapas
                        disp('Test restricciones proy excluyentes por etapas')
                        indice_ineq_desde = this.iIndiceIneq + 1;
                        indice_ineq_hasta = this.iIndiceIneq + this.iCantEtapas;
                        this.iIndiceIneq = indice_ineq_hasta;
                        for proy = 1:length(this.pAdmProy.ProyTransmisionExcluyentes(indice).Proyectos)
                            ...
                            indice_proy_desde = this.pAdmProy.entrega_proyecto(id_proy).entrega_varopt_expansion('Acumulada', escenario);
                            indice_proy_hasta = indice_proy_desde + this.iCantEtapas - 1;
                            this.agrega_valores_matriz_desigualdad(this.iCantEtapas,indice_ineq_desde:indice_ineq_hasta, indice_proy_desde:indice_proy_hasta,1);
                        ...
                        this.bineq(indice_ineq_desde:indice_ineq_hasta,1) = 1;
                        this.TipoRestriccionIneq(indice_ineq_desde:indice_ineq_hasta) = 1;
                        this.EscenarioRestriccionIneq(indice_ineq_desde:indice_ineq_hasta) = escenario;
                        this.EtapaRestriccionIneq(indice_ineq_desde:indice_ineq_hasta) = 1:1:this.iCantEtapas;
                        this.SubetapaRestriccionIneq(this.iIndiceIneq) = 0;
                        if this.iNivelDebug > 1 
                            etapas = (1:1:this.iCantEtapas)';
                            indices_ineq = (indice_ineq_desde:1:indice_ineq_hasta)';
                            nombre = strcat('rineq_', num2str(indices_ineq), '_proy_excl_', num2str(indice), '_S', num2str(escenario), '_E', num2str(etapas,'%02d'));
                            this.NombreIneq(indices') = cellstr(nombre);
                        end
                    else
                        this.iIndiceIneq = this.iIndiceIneq + 1;
...
%                     end
		...
		function optimiza(this)
            ...
            elseif strcmp(this.pParOpt.EstrategiaOptimizacion,'Benders')
                %if this.pParOpt.NivelSubetapasParaCortes && this.pParOpt.ConsideraAlphaMin
                %    this.calcula_coper_sin_restricciones_subetapas();                        
                %elseif this.pParOpt.ConsideraAlphaMin
                %    this.calcula_coper_sin_restricciones_antiguo();
                %end
                ...
                estrategia = 1;
                if estrategia == 1                    
                    %if ~this.pParOpt.NivelSubetapasParaCortes                    
                    %    if this.pParOpt.ComputoParalelo
                    %        this.optimiza_benders_paralelo_antiguo();
                    %    else
                    %        this.optimiza_benders_secuencial_antiguo();
                    %    end
                    %else
                    %    if this.pParOpt.ComputoParalelo
        ...
        function optimiza_benders_paralelo_antiguo(this)
            ...
            if length(cant_expansion_en_slave) > 1
                error = MException('cOptMILP:optimiza_benders_paralelo_antiguo',...
        ...
        function optimiza_benders_secuencial_antiguo(this)
            options = xprsoptimset('OUTPUTLOG',0);
            if this.iNivelDebug > 1
                prot = cProtocolo.getInstance;
            end
...
% for j = 1:this.iCantEscenarios*this.iCantEtapas
%     lb_alphas_gap = this.COperUninodal/pesos_alpha(j);
% end
            ub_master = [ub_master inf*ones(1,this.iCantEscenarios*this.iCantEtapas)];
            ctype_master = [ctype_master repmat('C', [1 this.iCantEscenarios*this.iCantEtapas])];
...
%                num2str(round(z_lower(k),5)), num2str(round((best_z_upper-z_lower(k))/best_z_upper*100,5)), num2str(round((best_z_upper-z_lower_gap(k))/best_z_upper*100,5)), num2str(dt_master), num2str(dt_slave), num2str(dt_total));
        ...
        function optimiza_benders_secuencial(this)
            ...
            best_z_upper = inf;
            best_sol = 0;
            texto = sprintf('%-5s %-5s %-15s %-15s %-15s %-15s %-10s %-10s %-10s %-10s','It', 'Sol', 'ZUpper(k)', 'ZUpperB', 'ZLower(k)', 'Gap%', '#Cortes', 'Dtmaster', 'Dtslave', 'DtTotal');
            disp(texto)
...
% linlinlin = 1
                ...
                dt_total = etime(clock, t_inicio);
                texto = sprintf('%-5s %-5s %-15s %-15s %-15s %-15s %-10s %-10s %-10s %-10s',num2str(k), texto_nueva_sol, num2str(round(z_upper(k),5)), num2str(round(best_z_upper,5)), ...
                    num2str(round(z_lower(k),5)), num2str(round((best_z_upper-z_lower(k))/best_z_upper*100,5)), num2str(length(bineq_cortes)), num2str(dt_master), num2str(dt_slave), num2str(dt_total));
                disp(texto)
        ...
        function optimiza_benders_paralelo(this)
            ...
            for j = 1:this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas
                ...
                peso_caso = pesos_alpha(j);
                matrices(j).fobj_slave = this.Fobj/peso_caso;
                matrices(j).fobj_slave(this.TipoVarOpt == 1) = 0;

                matrices(j).fobj_slave = matrices(j).fobj_slave(id_slave);
                matrices(j).lb_slave = this.lb(id_slave);
                matrices(j).ub_slave = this.ub(id_slave);
                
                matrices(j).a_slave = this.Aineq(this.TipoRestriccionIneq == 2 & this.EscenarioRestriccionIneq == escenario & this.EtapaRestriccionIneq == etapa & this.SubetapaRestriccionIneq == subetapa,id_slave);
                matrices(j).b_slave = this.bineq(this.TipoRestriccionIneq == 2 & this.EscenarioRestriccionIneq == escenario & this.EtapaRestriccionIneq == etapa & this.SubetapaRestriccionIneq == subetapa);

                cant_aineq = size(matrices(j).a_slave,1);
                
                matrices(j).a_slave = [matrices(j).a_slave; this.Aeq(this.TipoRestriccionEq == 2 & this.EscenarioRestriccionEq == escenario & this.EtapaRestriccionEq == etapa & this.SubetapaRestriccionEq == subetapa,id_slave)];
                matrices(j).b_slave = [matrices(j).b_slave; this.beq(this.TipoRestriccionEq == 2 & this.EscenarioRestriccionEq == escenario & this.EtapaRestriccionEq == etapa & this.SubetapaRestriccionEq == subetapa)];

                cant_aeq = size(matrices(j).a_slave,1)-cant_aineq;
                id_expansion_en_slave = id_master & id_slave;
                matrices(j).pos_sol_expansion_para_slave = find(id_expansion_en_slave); % posiciones de solución de expansión en variable y_master
                matrices(j).cant_expansion_en_slave = length(matrices(j).pos_sol_expansion_para_slave);
                
                sol_expansion = y_master(matrices(j).pos_sol_expansion_para_slave);
                
                %cant_expansion_en_slave = length(pos_sol_expansion_para_slave);
                matrices(j).pos_eq_sol_para_slave = 1+ size(matrices(j).a_slave,1);
                matrices(j).a_slave = [matrices(j).a_slave; zeros(matrices(j).cant_expansion_en_slave, length(matrices(j).fobj_slave))];
                
                matrices(j).a_slave(matrices(j).pos_eq_sol_para_slave:matrices(j).pos_eq_sol_para_slave+matrices(j).cant_expansion_en_slave-1, ...
                    1:matrices(j).cant_expansion_en_slave) = diag(ones(matrices(j).cant_expansion_en_slave,1));
                matrices(j).b_slave = [matrices(j).b_slave; sol_expansion];

                matrices(j).rtype_slave = [repmat('L',[1 cant_aineq]) repmat('E',[1 cant_aeq]) repmat('E',[1 matrices(j).cant_expansion_en_slave])];                
            end
            cant_expansion_en_slave = matrices(1).cant_expansion_en_slave;
            for j = 2:1:this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas
                if matrices(j).cant_expansion_en_slave ~= cant_expansion_en_slave
                    error = MException('cOptMILP:optimiza_benders_paralelo',...
                        'Cantidad de variables expansión difere en las distintas etapas. No se puede simular en forma paralela');
                    throw(error)                    
                end
            end
tiempos_totales_1 = zeros(this.pParOpt.MaxIterBenders, this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas);
tiempos_totales_2 = zeros(this.pParOpt.MaxIterBenders, this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas);
tiempos_totales_3 = zeros(this.pParOpt.MaxIterBenders, this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas);
tiempos_totales_post = zeros(this.pParOpt.MaxIterBenders, 1);
            for k = 1:this.pParOpt.MaxIterBenders
                if k > 1
                    % resuelve problema master con los cortes
                    t_inicio_master = clock;
...
%                        prot.imprime_vector_texto(nombre_validos(sol == 1)', ['\nSol. expansion en iteracion ' num2str(k)]);
                ...
                t_inicio_slave = clock;
tiempos1 = zeros(1, this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas);
tiempos2 = zeros(1, this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas);
tiempos3 = zeros(1, this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas);

                parfor j = 1:this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas
t_inicio_parfor = clock;
                    peso_caso = pesos_alpha(j);
                    sol_expansion = y_master(matrices(j).pos_sol_expansion_para_slave);
                    matrices(j).b_slave(matrices(j).pos_eq_sol_para_slave:end) = sol_expansion;
tiempos1(j) = etime(clock,t_inicio_parfor);
t_inicio_opt = clock;
                    [x_slave,fval_slave,flag_slave,output_slave, lambda_slave] = xprslp(matrices(j).fobj_slave, ...
                        matrices(j).a_slave, matrices(j).b_slave,matrices(j).rtype_slave, matrices(j).lb_slave,matrices(j).ub_slave,options);
tiempos2(j) = etime(clock,t_inicio_opt);
t_inicio_post = clock;
                    % agrega costos de operación a z_upper
                    fval_slave_oper(j) = fval_slave*peso_caso;

                    lambda = round(lambda_slave.lin(end-matrices(j).cant_expansion_en_slave+1:end),dec_redondeo);
                    bineq_cortes_oper(j,1) = fval_slave + lambda'*sol_expansion;
                    lambda_aineq_cortes_oper(j,:) = lambda;
                    pos_expansion_en_slave_cortes_oper(j,:) = matrices(j).pos_sol_expansion_para_slave;
tiempos3(j) = etime(clock,t_inicio_post);
...
%                         end
%                         nombres_restricciones = [nombres_restricciones; nombres_eq_fija_exp_en_slave'];
%                             this.imprime_problema_optimizacion_elementos_dados(matrices(j).fobj_slave, matrices(j).a_slave, matrices(j).b_slave, matrices(j).rtype_slave, matrices(j).lb_slave, matrices(j).ub_slave, [], nombres_variables, nombres_restricciones, ['lp_par_slave_It' num2str(k) '_S' num2str(escenario) '_E' num2str(etapa) '_SE' num2str(subetapa)]);
%                     end                    
                end
t_inicio_post_processing = clock;                
                z_upper(k) = z_upper(k) + sum(fval_slave_oper);
                for j = 1:this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas
                    cant_cortes = cant_cortes + 1;
                    %pos_id_expansion_slave_en_master = find(id_expansion_en_slave_cortes(j,:));
...
tiempos_totales_1(k,:) = tiempos1;
tiempos_totales_2(k,:) = tiempos2;
tiempos_totales_3(k,:) = tiempos3;
tiempos_totales_post(k) = etime(clock,t_inicio_post_processing);
                if this.iNivelDebug > 3
                    % verifica que suma de bineq de cortes en cada
                    % escenario y etapa coincida con solucion completa
                    fobj_slave = this.Fobj;
        ...
        function optimiza_benders_paralelo_nuevo(this)
            options = xprsoptimset('OUTPUTLOG',0);
            if this.iNivelDebug > 2
                prot = cProtocolo.getInstance;
            end
...
%t_inicio_post_processing = clock;
                z_upper(k) = z_upper(k) + sum(fval_slave_oper);
                for j = 1:this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas
                    cant_nz = length(find(lambda_aineq_cortes_oper(j,:)));
                        cant_cortes = cant_cortes + 1;
                        indices = lambda_aineq_cortes_oper(j,:) ~= 0;
                        FilasCortes = [FilasCortes; cant_cortes*ones(cant_nz,1)];
                        ColCortes = [ColCortes; matrices(j).pos_sol_expansion_para_slave(indices)'];
                        ValCortes = [ValCortes; lambda_aineq_cortes_oper(j,indices)'];
                        FilasCortes = [FilasCortes; cant_cortes];
                        ColCortes = [ColCortes; cant_master + j];
                        ValCortes = [ValCortes; 1];
                        %bineq_cortes = [bineq_cortes; bineq_cortes_oper(j)];
                end

                aineq_cortes = sparse(FilasCortes,ColCortes,ValCortes,cant_cortes,length(fobj_master));
                bineq_cortes = [bineq_cortes; bineq_cortes_oper];
                
                [cortes_sin_duplicado, indices_validos, ~] = unique([aineq_cortes bineq_cortes],'rows');
                aineq_cortes = cortes_sin_duplicado(:,1:end-1);
                %bineq_cortes = full(cortes_sin_duplicado(:,end));
                bineq_cortes = bineq_cortes(indices_validos);
                cant_cortes = length(bineq_cortes);
                [FilasCortes, ColCortes, ValCortes] = find(aineq_cortes); 
                if this.iNivelDebug > 2
                    nombres_cortes = [nombres_cortes; nombres_cortes_oper];
                end
                dt_slave = etime(clock,t_inicio_slave);
...
%tiempos_totales_post(k) = etime(clock,t_inicio_post_processing);
        ...
        function optimiza_benders_paralelo_nuevo2(this)
            options = xprsoptimset('OUTPUTLOG',0);
            if this.iNivelDebug > 0
                prot = cProtocolo.getInstance;
            end
            ...
            % define contenedores para los cortes
            bineq_cortes = zeros(this.pParOpt.MaxIterBenders*this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas,1); % cota superior
            cant_cortes = 0;
%            aineq_cortes = zeros(0, length(fobj_master));
            FilasCortes = zeros(this.pParOpt.MaxIterBenders*cant_expansion_en_slave*this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas,1);
            ColCortes = zeros(this.pParOpt.MaxIterBenders*cant_expansion_en_slave*this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas,1);
            ValCortes = zeros(this.pParOpt.MaxIterBenders*cant_expansion_en_slave*this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas,1);
            cant_nz_cortes = 0;
            ...
            for k = 1:this.pParOpt.MaxIterBenders
                if k > 1
                    ...
                    if this.iNivelDebug > 2
                        ...
                        nombres_restricciones = [nombres_restricciones_master ; nombres_cortes];
                        if this.iNivelDebug > 3
                            this.imprime_problema_optimizacion_elementos_dados(fobj_master, [aineq_master; aeq_master; aineq_cortes], [bineq_master; beq_master; bineq_cortes(1:cant_cortes)], rtype_master, lb_master, ub_master, intcon_master, nombres_variables, nombres_restricciones, ['milp_master_' num2str(k)]);
                        end
                    end

                    [y_master,fval_master,exit_master,output_master] = xprsmip(fobj_master, [aineq_master; aeq_master;aineq_cortes], [bineq_master; beq_master; bineq_cortes(1:cant_cortes)],rtype_master, ctype_master, [], [], lb_master,ub_master, [], options);%, options);
                    dt_master = etime(clock,t_inicio_master);
                    y_master = round(y_master, dec_redondeo);
                    z_lower(k) = fval_master;
...
%                        prot.imprime_vector_texto(nombre_validos(sol == 1)', ['\nSol. expansion en iteracion ' num2str(k)]);
                    ...
                    z_lower(k) = 0;                    
%evalua solución inicial
% lb_inicial = lb_master;
% ub_inicial = ub_master;
% etapas_iniciales = [4 5 5 7 8];
% proy_iniciales = [24 38 121 56 11];
% for ii = 1:length(this.VarExpansion)
% 	id_var = this.VarExpansion(ii).entrega_indice();
%     id_expansion_desde = this.VarExpansion(ii).entrega_varopt_expansion('Decision', 1);
%     id_expansion_hasta = id_expansion_desde + this.iCantEtapas - 1;
%     ub_optima(id_expansion_desde:id_expansion_hasta) = 0;
%     if ~isempty(find(ismember(proy_iniciales, id_var), 1))
%         indice_opt = find(ismember(proy_iniciales, id_var));
%         etapa = etapas_iniciales(indice_opt);
%         lb_inicial(id_expansion_desde + etapa - 1) = 1;
%         ub_inicial(id_expansion_desde + etapa - 1) = 1;
%     end
% end
% [y_master,fval_master,exit_master,output_master] = xprsmip(fobj_master, [aineq_master; aeq_master], [bineq_master; beq_master],rtype_master, ctype_master, [], [], lb_inicial,ub_inicial, [],options);%, options);
% y_master = round(y_master, dec_redondeo);
% z_lower(k) = fval_master;
...
%t_inicio_post_processing = clock;
                z_upper(k) = z_upper(k) + sum(fval_slave_oper);
                cant_cortes_base = cant_cortes;
                for j = 1:this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas
                    cant_nz = length(find(lambda_aineq_cortes_oper(j,:)));
                    cant_cortes = cant_cortes + 1;
                    indices = lambda_aineq_cortes_oper(j,:) ~= 0;
                    FilasCortes(cant_nz_cortes+1:cant_nz_cortes+cant_nz) = cant_cortes*ones(cant_nz,1);
                    ColCortes(cant_nz_cortes+1:cant_nz_cortes+cant_nz) = matrices(j).pos_sol_expansion_para_slave(indices)';
                    ValCortes(cant_nz_cortes+1:cant_nz_cortes+cant_nz) = lambda_aineq_cortes_oper(j,indices)';
                    FilasCortes(cant_nz_cortes+cant_nz+1) = cant_cortes;
                    ColCortes(cant_nz_cortes+cant_nz+1) = cant_master + j;
                    ValCortes(cant_nz_cortes+cant_nz+1) = 1;
                    cant_nz_cortes = cant_nz_cortes+cant_nz+1;
                    %bineq_cortes = [bineq_cortes; bineq_cortes_oper(j)];
                end

                aineq_cortes = sparse(FilasCortes(1:cant_nz_cortes),ColCortes(1:cant_nz_cortes),ValCortes(1:cant_nz_cortes),cant_cortes,length(fobj_master));
                bineq_cortes(cant_cortes_base+1:cant_cortes) = bineq_cortes_oper;
                
                [cortes_sin_duplicado, indices_validos, ~] = unique([aineq_cortes bineq_cortes(1:cant_cortes)],'rows');
                aineq_cortes = cortes_sin_duplicado(:,1:end-1);
                %bineq_cortes = full(cortes_sin_duplicado(:,end));
                bineq_cortes = bineq_cortes(indices_validos);
                cant_cortes = length(bineq_cortes);
                ...
                cant_nz_cortes = length(FilasCortes);
                dt_slave = etime(clock,t_inicio_slave);
                ...
                if (best_z_upper-z_lower(k))/best_z_upper < this.pParOpt.MaxGap
                    break
                end
%tiempos_totales_1(k,:) = tiempos1;
%tiempos_totales_2(k,:) = tiempos2;
...
%tiempos_totales_post(k) = etime(clock,t_inicio_post_processing);
            ...
            ctype = repmat('C', [1 size(this.Fobj,1)]);
            ctype(this.intcon) = 'B';
if length(this.beq) ~= size(this.Aeq,1)
    this.beq = this.beq(1:size(this.Aeq,1));
    warning('Dimension de vector de igualdad no coincide con indicada en container. Se corrige')
end
if length(this.bineq) ~= size(this.Aineq,1)
    this.bineq = this.bineq(1:size(this.Aineq,1));
    warning('Dimension de vector de desigualdad no coincide con indicada en container. Se corrige')
end
            [this.ResOptimizacion,this.Fval,this.ExitFlag,this.Output] = xprsmip(this.Fobj,[this.Aineq; this.Aeq], [this.bineq; this.beq], rtype, ctype, [], [], this.lb,this.ub, []);%, options);
        end
...
%                    end
                    ...
                    for proy = 1:length(this.pAdmProy.ProyTransmision)    
                        ...
                        if val > 0
                            ...
                            if this.pParOpt.considera_valor_residual_elementos()
                                ...
                                costo_inversion_total = costo_inv*q^(detapa*etapa);
                            else
                            ...
                            costo_inversion{escenario}(etapa) = costo_inversion{escenario}(etapa) + costo_inversion_total;
                        end
                    end
                end
                for etapa = 1:this.iCantEtapas
                ...
                if this.pParOpt.considera_recorte_res()
                    ...
                    for i = 1:length(generadores_res)
                        ...
                        for etapa = 1:this.iCantEtapas
                            ...
                            rep = this.pAdmSc.RepresentatividadPuntosOperacion;
                            recorte_res{escenario}(etapa) = recorte_res{escenario}(etapa) + sum(sbase*recorte.*rep);
                ...
                if this.pParOpt.considera_desprendimiento_carga()
                    ...
                    for i = 1:length(consumos)
                        ...
                        for etapa = 1:this.iCantEtapas
                            ...
                            rep = this.pAdmSc.RepresentatividadPuntosOperacion;
                            ens{escenario}(etapa) = ens{escenario}(etapa) + sum(sbase*ens_consumo.*rep);
        ...
        function exporta_resultados_formato_excel(this)
            prot = cProtocolo.getInstance; 
            %prot.imprime_matriz_formato_excel(matriz resultados, heathers, nombre_matriz, nombre_archivo, nombre_hoja);
            %n_se = length(this.pSEP.Subestaciones)+length(this.pAdmProy.entrega_subestaciones_expansion());
            %n_escenarios = this.iCantEscenarios;
            n_bat = length(this.pSEP.Baterias)+length(this.pAdmProy.entrega_baterias_expansion());
            n_lineas_trafos = length(this.pSEP.entrega_lineas())+length(this.pSEP.entrega_transformadores2d())+length(this.pAdmProy.entrega_elementos_serie_expansion());
            n_gen_desp = length(this.pSEP.entrega_generadores_despachables());
            n_gen_res = length(this.pSEP.entrega_generadores_res())+length(this.pAdmProy.entrega_generadores_ernc_proyectados_todos());
            n_gen = n_gen_desp + n_gen_res;
            n_cons = length(this.pSEP.entrega_consumos());
            cant_po = this.iCantPuntosOperacion;
            for i = 1:this.iCantEscenarios*this.iCantEtapas
                [escenario, etapa] = ind2sub([this.iCantEscenarios, this.iCantEtapas],i);
                Resultados = cell(n_gen+n_cons+n_lineas_trafos+n_bat+n_gen_res+n_cons,3+cant_po);
                gen_desp = this.pSEP.entrega_generadores_despachables();
                for j = 1:length(gen_desp)
                    indice_desde = gen_desp(j).entrega_varopt_operacion('P', escenario, etapa);
                    indice_hasta = indice_desde + cant_po - 1;
                    
                    Resultados{j,1} = gen_desp(j).entrega_nombre();
                    Resultados(j,2) = num2cell(max(this.ub(indice_desde:indice_hasta)));
                    Resultados(j,3) = num2cell(max(this.ResOptimizacion(indice_desde:indice_hasta)));
                    Resultados(j,4:end) = num2cell(this.ResOptimizacion(indice_desde:indice_hasta)');
                end
                gen_res = [this.pSEP.entrega_generadores_res(); this.pAdmProy.entrega_generadores_ernc_proyectados_todos()'];
                fila_actual = n_gen_desp;
                for j = 1:length(gen_res)
                    indice_desde = gen_res(j).entrega_varopt_operacion('P', escenario, etapa);
                    indice_hasta = indice_desde + cant_po - 1;
                    
                    fila_actual = fila_actual + 1;
                    Resultados{fila_actual,1} = gen_res(j).entrega_nombre();
                    if indice_desde > 0
                        Resultados{fila_actual,2} = '-';
                        Resultados(fila_actual,3) = num2cell(max(this.ub(indice_desde:indice_hasta)));
                        Resultados(fila_actual,4:end) = num2cell(this.ub(indice_desde:indice_hasta)');
                    end
                    fila_actual = fila_actual + 1;
                    Resultados{fila_actual,1} = ['Spill_' gen_res(j).entrega_nombre()];
                    if indice_desde > 0
                        Resultados{fila_actual,2} = '-';
                        Resultados(fila_actual,3) = num2cell(max(this.ResOptimizacion(indice_desde:indice_hasta)));
                        Resultados(fila_actual,4:end) = num2cell(this.ResOptimizacion(indice_desde:indice_hasta)');
                    end
                end

                cons = this.pSEP.entrega_consumos();
                for j = 1:length(cons)
                    indice_desde = cons(j).entrega_varopt_operacion('P', escenario, etapa);
                    indice_hasta = indice_desde + cant_po - 1;
                    
                    fila_actual = fila_actual + 1;
                    Resultados{fila_actual,1} = cons(j).entrega_nombre();
                    Resultados{fila_actual,2} = '';
                    Resultados(fila_actual,3) = num2cell(max(this.ub(indice_desde:indice_hasta)));
                    Resultados(fila_actual,4:end) = num2cell(this.ub(indice_desde:indice_hasta)');

                    fila_actual = fila_actual + 1;
                    Resultados{fila_actual,1} = ['ENS_' cons(j).entrega_nombre()];
                    Resultados{fila_actual,2} = '-';
                    Resultados(fila_actual,3) =  num2cell(max(this.ResOptimizacion(indice_desde:indice_hasta)));
                    Resultados(fila_actual,4:end) = num2cell(this.ResOptimizacion(indice_desde:indice_hasta)');
                end

                el_red = [this.pSEP.entrega_lineas(); this.pSEP.entrega_transformadores2d(); this.pAdmProy.entrega_elementos_serie_expansion()];
                for j = 1:length(el_red)
                    indice_desde = el_red(j).entrega_varopt_operacion('P', escenario, etapa);
                    indice_hasta = indice_desde + cant_po - 1;
                    
                    fila_actual = fila_actual + 1;
                    Resultados{fila_actual,1} = el_red(j).entrega_nombre();
                    Resultados(fila_actual,2) = num2cell(max(this.ub(indice_desde:indice_hasta)));
                    Resultados(fila_actual,3) = num2cell(max(abs(this.ResOptimizacion(indice_desde:indice_hasta))));
                    Resultados(fila_actual,4:end) = num2cell(this.ResOptimizacion(indice_desde:indice_hasta)');
                end

                heathers = cell(1, cant_po + 3);
                heathers{1} = 'Nombres';
                heathers{2} = 'MaxNom';
                heathers{3} = 'MaxRes';
                heathers(4:end) = num2cell(1:1:cant_po);
                prot.imprime_matriz_formato_excel(Resultados', heathers, sprintf('Resultados operacion'), ['Res_' this.pParOpt.Output], ['S' num2str(escenario) '_E' num2str(etapa)]);
            end
        end
                        ...
                        for bat = 1:length(baterias)
                            ...
                            if ~existente
                                if p_bat_descarga ~= 0 || p_bat_carga ~= 0
                                    ...
                                    texto = [texto baterias(bat).entrega_nombre()];
                                    texto = [texto ' tiene potencia distinta de cero. PBateria: ' num2str(p_bat_carga+p_bat_descarga)];
                                    %warning(texto)
                                    error = MException('cOptMILP:imprime_resultados_protocolo',texto);
                                    throw(error)
                                end
...
% METODO ORIGINAL QUE UTILIZA COMBVEC (NO DISPONIBLE EN LEFTRARU)                        
%                         if cant_grupos_adyacentes == 0
%                             combinaciones_test = [];
%                         elseif cant_grupos_adyacentes == 1
%                             combinaciones_test = combvec(grupos_id_proy_adyacentes{1});
%                         elseif cant_grupos_adyacentes == 2
%                             combinaciones_test = combvec(grupos_id_proy_adyacentes{1}, grupos_id_proy_adyacentes{2});
%                         elseif cant_grupos_adyacentes == 3
%                             combinaciones_test = combvec(grupos_id_proy_adyacentes{1}, grupos_id_proy_adyacentes{2}, grupos_id_proy_adyacentes{3});
%                         elseif cant_grupos_adyacentes == 4
%                             combinaciones_test = combvec(grupos_id_proy_adyacentes{1}, grupos_id_proy_adyacentes{2}, grupos_id_proy_adyacentes{3}, grupos_id_proy_adyacentes{4});
%                         elseif cant_grupos_adyacentes == 5
%                             combinaciones_test = combvec(grupos_id_proy_adyacentes{1}, grupos_id_proy_adyacentes{2}, grupos_id_proy_adyacentes{3}, grupos_id_proy_adyacentes{4}, grupos_id_proy_adyacentes{5});
%                         elseif cant_grupos_adyacentes == 6
%                             combinaciones_test = combvec(grupos_id_proy_adyacentes{1}, grupos_id_proy_adyacentes{2}, grupos_id_proy_adyacentes{3}, grupos_id_proy_adyacentes{4}, grupos_id_proy_adyacentes{5}, grupos_id_proy_adyacentes{6});
%                         elseif cant_grupos_adyacentes == 7
%                             combinaciones_test = combvec(grupos_id_proy_adyacentes{1}, grupos_id_proy_adyacentes{2}, grupos_id_proy_adyacentes{3}, grupos_id_proy_adyacentes{4}, grupos_id_proy_adyacentes{5}, grupos_id_proy_adyacentes{6}, grupos_id_proy_adyacentes{7});
%                         elseif cant_grupos_adyacentes == 8
%                             combinaciones_test = combvec(grupos_id_proy_adyacentes{1}, grupos_id_proy_adyacentes{2}, grupos_id_proy_adyacentes{3}, grupos_id_proy_adyacentes{4}, grupos_id_proy_adyacentes{5}, grupos_id_proy_adyacentes{6}, grupos_id_proy_adyacentes{7}, grupos_id_proy_adyacentes{8});
%                         else
...
%            fprintf(this.docIDFico, ['results: array(1..' num2str(this.iCantVarDecision) ') of real\n']);
        ...
        function calcula_flujos_maximos_corredores(this)        
                ...
                if isa(this.VarOperacion(i), 'cLinea') || isa(this.VarOperacion(i), 'cTransformador2D')
                    for escenario = 1:this.iCantEscenarios
                        for etapa = 1:this.iCantEtapas
                            ...
                            lb_oper(indice_desde:indice_hasta) = 100*lb_oper(indice_desde:indice_hasta);
                            ub_oper(indice_desde:indice_hasta) = 100*ub_oper(indice_desde:indice_hasta);
                        end
                    end
                end
            end
            ...
            fobj_sr = this.Fobj;
            fobj_sr(this.TipoVarOpt == 1) = 0;
            lb_sr = this.lb;
            ub_sr = this.ub;
            lb_sr(this.TipoVarOpt == 1) = 0;
            ub_sr(this.TipoVarOpt == 1) = 0;

            % proyectos obligatorios
            for i = 1:length(this.pAdmProy.ProyTransmisionObligatorios)
                for escenario = 1:this.iCantEscenarios
                    for j = 1:length(this.pAdmProy.ProyTransmisionObligatorios(i).Proyectos)
                        % se ejecutan todos los proyectos obligatorios. Se
                        % asume que no hay uprating, i.e. no hay proyectos
                        % excluyentes entre sí
                        %indice_proy_desde = this.pAdmProy.ProyTransmisionObligatorios(i).Proyectos(j).entrega_varopt_expansion('Decision', escenario);
                        %indice_proy_hasta = indice_proy_desde + this.iCantPuntosOperacion - 1;
                        %lb_sr(indice_proy_desde) = 1;
                        %ub_sr(indice_proy_desde) = 1;
                        
                        % decision acumulada
                        indice_cum_desde = this.pAdmProy.ProyTransmisionObligatorios(i).Proyectos(j).entrega_varopt_expansion('Acumulada', escenario);
                        indice_cum_hasta = indice_cum_desde + this.iCantPuntosOperacion - 1;
                        lb_sr(indice_cum_desde:indice_cum_hasta) = 1;
                        ub_sr(indice_cum_desde:indice_cum_hasta) = 1;
                        
                    end
                end
            end
            
            aineq_sr = this.Aineq(this.TipoRestriccionIneq == 2,:);
            bineq_sr = this.bineq(this.TipoRestriccionIneq == 2);

            aeq_sr = this.Aeq(this.TipoRestriccionEq == 2,:);
            beq_sr = this.beq(this.TipoRestriccionEq == 2);

            rtype_sr = [repmat('L',[1 size(aineq_sr,1)]) repmat('E',[1 size(aeq_sr,1)])];

            [this.ResOptimizacion,fval,exit_flag,output, lambda] = xprslp(fobj_sr,[aineq_sr; aeq_sr], [bineq_sr; beq_sr], rtype_sr, lb_sr,ub_sr, []);%, options);
            this.imprime_resultados_protocolo();
            this.escribe_resultados();
        end

        function coper_uninodal = calcula_coper_sin_restricciones(this)
            t_inicio = clock;
            %this.COperUninodal = zeros(1,this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas);
            coper_uninodal = zeros(1,this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas);
            % límites de variables de decisión se ponen a cero
            lb_oper = this.lb;
            ub_oper = this.ub;
            ...
            id_vars = this.TipoVarOpt == 2;
            fobj_oper_sr = this.Fobj(id_vars);
            lb_oper = lb_oper(id_vars);
            ub_oper = ub_oper(id_vars);
            escenario_vars = this.EscenarioVarOpt(id_vars);
            etapas_vars = this.EtapaVarOpt(id_vars);
            subetapas_vars = this.SubetapaVarOpt(id_vars);
            [Aeq_oper_sr, beq_oper_sr] = this.entrega_rest_be_uninodal();
            Aeq_oper_sr = Aeq_oper_sr(:,id_vars);
            
            if strcmp(this.pParOpt.Solver, 'Intlinprog')
                            error = MException('cOptMILP:calcula_coper_sin_restricciones','Por ahora solo Xpress');
                            throw(error)
            elseif strcmp(this.pParOpt.Solver, 'Xpress')
                %options= xprsoptimset('MAXMEMORY',this.pParOpt.MaxMemory,'MAXTIME',this.pParOpt.MaxTime);
                %if this.pParOpt.MaxGap > 0
                ...
                %end
                options = xprsoptimset('OUTPUTLOG',0);
                rtype = repmat('E',[1 size(Aeq_oper_sr,1)]);
                [x_oper,fval,exitflag,output, lambda] = xprslp(fobj_oper_sr, Aeq_oper_sr, beq_oper_sr,rtype, lb_oper,ub_oper,options);%, options);
            end

            for j = 1:this.iCantEscenarios*this.iCantEtapas*this.iCantSubetapas
                [escenario, etapa, subetapa] = ind2sub([this.iCantEscenarios, this.iCantEtapas, this.iCantSubetapas],j);
                id_vars_caso = escenario_vars == escenario & etapas_vars == etapa & subetapas_vars == subetapa;
                
                %this.COperUninodal(j) = fobj_oper_sr(id_vars_caso)'*x_oper(id_vars_caso);
                coper_uninodal(j) = fobj_oper_sr(id_vars_caso)'*x_oper(id_vars_caso);
            end
        ...
        function calcula_coper_sin_restricciones_antiguo(this)
            t_inicio = clock;
        ...
        function [Aeq_oper_sr, beq_oper_sr] = entrega_rest_be_uninodal(this)
            
            cant_variables_decision = size(this.Aeq,2);
            cant_po = this.iCantEscenarios*this.iCantEtapas*this.iCantPuntosOperacion;
            %Aeq_oper_sr = sparse(cant_po, cant_variables_decision);
            beq_oper_sr = zeros(cant_po,1);
            indices_filas = [];
            indices_col = [];
            ...
            for escenario = 1:this.iCantEscenarios
                for etapa = 1:this.iCantEtapas
                    id_be_uninodal_desde = (escenario - 1)*this.iCantPuntosOperacion*this.iCantEtapas + (etapa -1)*this.iCantPuntosOperacion + 1;
                    id_be_uninodal_hasta = (escenario -1)*this.iCantPuntosOperacion*this.iCantEtapas + etapa*this.iCantPuntosOperacion;
                    ...
                    for se = 1:length(subestaciones)
                        id_be_desde = subestaciones(se).entrega_restriccion_balance_energia_desde(escenario, etapa);
                        id_be_hasta = id_be_desde + this.iCantPuntosOperacion - 1;
                        %Aeq_oper_sr(id_be_uninodal_desde:id_be_uninodal_hasta) = Aeq_oper_sr(id_be_uninodal_desde:id_be_uninodal_hasta) + this.Aeq(id_be_desde:id_be_hasta,:);
                        [filas, columnas, valores] = find(this.Aeq(id_be_desde:id_be_hasta,:));
                        filas = filas + id_be_uninodal_desde - 1;
                        beq_oper_sr(id_be_uninodal_desde:id_be_uninodal_hasta) = beq_oper_sr(id_be_uninodal_desde:id_be_uninodal_hasta) + this.beq(id_be_desde:id_be_hasta);
                        indices_filas = [indices_filas; filas];
                        indices_col = [indices_col; columnas];
                        indices_val = [indices_val; valores];
                    end
                end
            end
            Aeq_oper_sr = sparse(indices_filas,indices_col,indices_val,cant_po,cant_variables_decision);
        end
        ...
        function nombre = entrega_nombre_proyecto_expansion(this, proy, escenario, id_var_expansion)
            etapa = (1:1:this.iCantEtapas)';
            ...
            if strcmp(tipo_proyecto, 'AL')
                ...
                nombre = strcat('Pr_', num2str(id_var_expansion), '_AL_', num2str(id_par), '_C', num2str(tipo_cond), '_CS', num2str(compensacion), '_V', num2str(vn), ...
            ...
            elseif strcmp(tipo_proyecto, 'AB')
                ...
                nombre = strcat('Pr_', num2str(id_var_expansion), '_AB_', num2str(id_par), '_C', num2str(tipo_bat), '_V', num2str(vn), ...
...
%                             nombre = ['Pr_' num2str(i) '_CC_' num2str(id_par) '_' cap_inicial '_' cap_final '_CS' num2str(compensacion) '_V' num2str(vn) ...
%                                       '_B' num2str(id_ubic_1) '_' num2str(id_ubic_2) '_E' num2str(etapa) '_S' num2str(escenario)];
                nombre = strcat('Pr_', num2str(id_var_expansion), '_CC_', num2str(id_par), '_', cap_inicial, '_', cap_final, '_CS', num2str(compensacion), '_V', num2str(vn), ...
                          '_B', num2str(id_ubic_1), '_', num2str(id_ubic_2), '_E', num2str(etapa,'%02d'), '_S', num2str(escenario));
            elseif strcmp(tipo_proyecto, 'AV')
                ...
                nombre = strcat('Pr_', num2str(id_var_expansion), '_AV_', num2str(id_par), '_', cap_inicial, '_', cap_final, '_C', num2str(tipo_conductor), ...
                          '_B', num2str(id_ubic_1), '_', num2str(id_ubic_2), '_E', num2str(etapa,'%02d'), '_S', num2str(escenario));
            elseif strcmp(tipo_proyecto, 'CS')
                ...
                vn = proy.Elemento(end).entrega_se1().entrega_vn();
                nombre = strcat('Pr_', num2str(id_var_expansion), '_CS_', num2str(id_par), '_', cap_inicial, '_', cap_final, '_C', num2str(tipo_conductor), '_V', num2str(vn), ...
                          '_B', num2str(id_ubic_1), '_', num2str(id_ubic_2), '_E', num2str(etapa,'%02d'), '_S', num2str(escenario));
            elseif strcmp(tipo_proyecto, 'AT')
                ...
                nombre = strcat('Pr_', num2str(id_var_expansion), '_AT_', num2str(id_par), '_T', num2str(tipo_trafo), '_Sr_', num2str(sr), '_V_', num2str(vat), '_', num2str(vbt), ...
            ...
            elseif strcmp(tipo_proyecto, 'AS')
                ...
                nombre = strcat('Pr_', num2str(id_var_expansion), '_AS_B_', num2str(id_ubic), '_V_', num2str(vn), '_E', num2str(etapa,'%02d'), '_S', num2str(escenario));
            end
        end
		...
		function escribe_resultados(this)
            ...
            sbase = cParametrosSistemaElectricoPotencia.getInstance.entrega_sbase();
            for j = 1:this.iCantEscenarios*this.iCantEtapas
                this.pResEvaluacion{j} = cResultadoEvaluacionSEP(this.pSEP, 1, 3, cant_po); % 1 indica que es OPF
                this.pResEvaluacion{j}.ExisteResultadoOPF = true;
            end
            FlujoCorredores = zeros(0,3);
            
            % subestaciones
            subestaciones = this.pSEP.entrega_subestaciones();
            %this.pAdmProy.entrega_subestaciones_expansion()            
            for j = 1:this.iCantEscenarios*this.iCantEtapas
                [escenario, etapa] = ind2sub([this.iCantEscenarios, this.iCantEtapas],j);
                id_se_actual = 0;
                id_linea_actual = 0;
                id_trafo_actual = 0;
                id_generador_actual = 0;
                ...
                for i = 1:length(this.VarOperacion)
                    if isa(this.VarOperacion(i),'CSubestacion')
                        ...
                        id_se_actual = id_se_actual + 1;
                        this.pResEvaluacion{j}.AnguloSubestaciones(id_se_actual, :) = theta'/pi*180;
                    elseif isa(this.VarOperacion(i), 'cLinea')
                        indice_desde = this.VarOperacion(i).entrega_varopt_operacion('P', escenario, etapa);
                        indice_hasta = indice_desde + cant_po -1;
                        id_linea_actual = id_linea_actual + 1;
                        p_mw = this.ResOptimizacion(indice_desde:indice_hasta)*sbase;
                        this.pResEvaluacion{j}.FlujoLineasP(id_linea_actual, :) = p_mw';
                        if ~isempty(FlujoCorredores)
                            id_se_1 = this.VarOperacion(i).entrega_se1().entrega_id();
                            id_se_2 = this.VarOperacion(i).entrega_se2().entrega_id();
                            pos_corredor = find(ismember(FlujoCorredores(:,1:2), [id_se_1 id_se_2],'rows'));
                            signo = 1;
                            if isempty(pos_corredor)
                                pos_corredor = find(ismember(FlujoCorredores(:,1:2), [id_se_2 id_se_1],'rows'));
                                signo = -1;
                            end
                            
                            if isempty(pos_corredor)
                                n_corr = size(FlujoCorredores,1) + 1;
                                FlujoCorredores(n_corr,1) = id_se_1;
                                FlujoCorredores(n_corr,2) = id_se_2;
                                FlujoCorredores(n_corr,3:end) = p_mw;
                            else
                                FlujoCorredores(pos_corredor,3:end) = FlujoCorredores(pos_corredor,3:end) + signo*p_mw';
                            end
                        else
                            FlujoCorredores(1,1) = id_se_1;
                            FlujoCorredores(1,2) = id_se_2;
                            FlujoCorredores(1,3:cant_po + 2) = p_mw';                            
                        end
                    elseif isa(this.VarOperacion(i), 'cTransformador2D')
                        indice_desde = this.VarOperacion(i).entrega_varopt_operacion('P', escenario, etapa);
                        indice_hasta = indice_desde + cant_po -1;
                        id_trafo_actual = id_trafo_actual + 1;
                        p_mw = this.ResOptimizacion(indice_desde:indice_hasta)*sbase;
                        this.pResEvaluacion{j}.FlujoTransformadoresP(id_trafo_actual, :) = p_mw';
                        if ~isempty(FlujoCorredores)
                            id_se_1 = this.VarOperacion(i).entrega_se1().entrega_id();
                            id_se_2 = this.VarOperacion(i).entrega_se2().entrega_id();
                            pos_corredor = find(ismember(FlujoCorredores(:,1:2), [id_se_1 id_se_2],'rows'));
                    ...
                    elseif isa(this.VarOperacion(i), 'cGenerador')
                        indice_desde = this.VarOperacion(i).entrega_varopt_operacion('P', escenario, etapa);
                        indice_hasta = indice_desde + cant_po -1;
                        id_generador_actual = id_generador_actual + 1;
                        p_mw = this.ResOptimizacion(indice_desde:indice_hasta)*sbase;
                        if this.VarOperacion(i).es_despachable()
                            this.pResEvaluacion{j}.GeneradoresP(id_generador_actual, :) = p_mw';
                        else
                            val_nominal = this.ub(indice_desde:indice_hasta)*sbase;
                            this.pResEvaluacion{j}.GeneradoresP(id_generador_actual, :) = (val_nominal-p_mw)';
                            this.pResEvaluacion{j}.RecorteRES(id_generador_actual, :) = p_mw';
                        end
                    elseif isa(this.VarOperacion(i), 'cConsumo')
                        indice_desde = this.VarOperacion(i).entrega_varopt_operacion('P', escenario, etapa);
                        indice_hasta = indice_desde + cant_po -1;
                        id_consumo_actual = id_consumo_actual + 1;
                        val_nominal = this.ub(indice_desde:indice_hasta)*sbase;
                        p_mw = this.ResOptimizacion(indice_desde:indice_hasta)*sbase;
                        this.pResEvaluacion{j}.ENS(id_consumo_actual, :) = p_mw';
                        this.pResEvaluacion{j}.ConsumosP(id_consumo_actual, :) = (val_nominal - p_mw)';
                    end
                end
            end
        end
