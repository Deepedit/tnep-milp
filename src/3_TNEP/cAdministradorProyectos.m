classdef cAdministradorProyectos < handle
    ...
    properties
        ...
        % por cada corredor
        TipoDecisionPrimaria % 1: Líneas de transmisión/trafos, 2: Baterías (Inyección P), 3: Condensadores (Inyección Q positiva); 4: reactores (Inyección Q negativa), 5: Generación (Inyección P)
        MatrizEstadosDecisionPrimaria % cada corredor y bus (para baterías y generador) tiene una matriz de estados donde se indican los 
    ...
    methods
        ...
        function agrega_proyecto_primario(this, nuevo_proyecto, id_decision)
            ...
            this.RelProyIdDecisionExpansion(this.CantidadProyTransmision,1) = id_decision;
        ...
        function agrega_proyecto_auxiliar(this, nuevo_proyecto)
            ...
            this.RelProyIdDecisionExpansion(this.CantidadProyTransmision,1) = 0;
        end
        ...
        function [espacio_busqueda, primera_etapa_posible] = determina_espacio_busqueda(this, plan, varargin)
            ...
            for i=1:length(this.ProyTransmision)
                ...
                if strcmp(proy.entrega_tipo_proyecto(), 'AS') || ...
                        (strcmp(proy.entrega_tipo_proyecto(), 'AT') && proy.EsUprating && proy.Elemento(1).entrega_indice_paralelo() == 1)
                    % agrega subestación o primer transformador VU. Estos
        ...
        function [espacio_busqueda, primera_etapa_posible] = determina_espacio_busqueda_calculo_pardis(this, plan, varargin)
            % varargin indica si hay proyectos restringidos
            proyectos_restringidos = [];
            if nargin > 2
                proyectos_restringidos = varargin{1};
            ...
            for i=1:length(this.ProyTransmision)
                ...
                if strcmp(proy.entrega_tipo_proyecto(), 'AS') || ...
                    ...
                    % conectividad)
                    continue;
                elseif ~proy.EsUprating && proy.Elemento(1).entrega_indice_paralelo() > 1
                    continue;
                end
        ...
        function [espacio_busqueda, directo]= determina_espacio_busqueda_repara_plan(this, plan, nro_etapa, varargin)
            ...
            for i = 1:n
                ...
                if isempty(proyectos_disponibles)
                    ...
                    [id_proy_existente, ~] = plan.entrega_proyectos_implementados_de_lista_a_partir_de_etapa(proy, nro_etapa + 1);
...
%                     end
        ...
        function cantidad = entrega_cantidad_proyectos_con_mismo_requisito_conectividad(this, plan, nro_etapa, id_proy_conectividad)
			...
			proy_en_plan = plan.entrega_proyectos_acumulados(nro_etapa);
...
%                    proy = this.ProyTransmision(plan.Plan(i).Proyectos(j));
...
%            end
        ...
        function consistente = verifica_consistencia_plan(this, plan)
            ...
            for nro_etapa = 1:cantidad_etapas
                ...
                for nro_proy_etapa = 1:cantidad_proyectos
                    proyecto = this.ProyTransmision(plan.Plan(nro_etapa).Proyectos(nro_proy_etapa));
        ...
        function inserta_tipo_decision_primaria(this, tipo)
            this.TipoDecisionPrimaria = tipo;
        end
        ...
        function val = entrega_cantidad_decisiones_primarias_transmision(this)
            val = length(find(this.TipoDecisionPrimaria == 1));
        end

        function val = entrega_cantidad_decisiones_primarias_generacion(this)
            val = length(find(this.TipoDecisionPrimaria == 5));
        end
