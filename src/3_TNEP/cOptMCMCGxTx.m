classdef cOptMCMCGxTx < handle
    properties
        ...
        PlanOptimo = cPlanExpansionGxTx.empty  % plan óptimo calculado de otra parte e importado aquí
        FObjPlanOptimo                     % NPV Costos inversion + congestion
        TotexPlanOptimo                    % NPV Costos inversion + operacion
        PlanEvaluar = cPlanExpansionGxTx.empty  % plan para realizar pruebas
        
        %estructuras
        PlanesValidos

        % CapacidadDecisionesTxPorCadenas: por cada corredor, indica la
        % capacidad en cada etapa. NxM donde N = cant corr, es la cantidad de etapas. Por cada fila se guardan los
        % CapacidadDecisionesTxPorCadenas{2}(3,5) indica la capacidad del
        % corredor 2, para la cadena 3 en la etapa 5
        CapacidadDecisionesTxPorCadenas = []
        CantDecisionesTxPrimarias = 0
        CantDecisionesGx = 0
    ...
    methods
        function this = cOptMCMCGxTx(sep, AdmSc, adm_proyectos, par_optimizacion)
            this.pSEP = sep;
            this.pAdmSc = AdmSc;
            this.pAdmProy = adm_proyectos;
            this.pParOpt = par_optimizacion;
            this.CantDecisionesTxPrimarias = adm_proyectos.entrega_cantidad_decisiones_primarias_tx();
            this.CantDecisionesGx = adm_proyectos.entrega_cantidad_decisiones_primarias_gx();
        ...
        function optimiza(this)
            ...
            this.exporta_resultados(cadenas);
        end
        ...
        function cadenas = inicializa_cadenas(this)
            ...
            capacidad_inicial_corredores = this.pAdmProy.entrega_capacidad_inicial_tx_decisiones_primarias();
            for i = 1:cantidad_cadenas
                cadenas{i}.ProyTransmision = zeros(max_cantidad_pasos, this.pAdmProy.CantidadProyTransmision);
                cadenas{i}.ProyGeneracion = zeros(max_cantidad_pasos, this.pAdmProy.CantidadProyGeneracion);
                cadenas{i}.CambiosEstado = -1*ones(max_cantidad_pasos,1);
                cadenas{i}.Totex = zeros(max_cantidad_pasos,1);
                cadenas{i}.CinvTransmision = zeros(max_cantidad_pasos,1);
                cadenas{i}.CinvGeneracion = zeros(max_cantidad_pasos,1);
                cadenas{i}.COperacion = zeros(max_cantidad_pasos,1);                
                cadenas{i}.CapacidadDecisionesTx = ones(this.CantDecisionesTxPrimarias, cantidad_etapas).*capacidad_inicial_corredores';
                cadenas{i}.CapacidadDecisionesGx = zeros(this.CantDecisionesGx, cantidad_etapas);
                cadenas{i}.TiempoEnLlegarAlOptimo = 0;
                cadenas{i}.plan_actual = cPlanExpansionGxTx(i*max_cantidad_pasos*10 + 1);
            ...
            % estrategia para generar planes base. Plan de generación se
            % genera en forma aleatoria. Plan de transmisión en función de
            % la estrategia:
            %  1: greedy (agrega líneas/baterias sólo si hay ENS o recorte RES)
            %  2: aleatorio 
            %  3: mixto (porcentaje greedy/aleatorio)
            ...
            elseif this.pParOpt.EstrategiaGeneraPlanesBase < 1
                %mixto
                estrategia_genera_planes = ones(cantidad_cadenas,1);
                ...
                error = MException('cOptMCMC:inicializa_cadenas','Dato de entrada erroneo. Estrategia genera planes base debe ser <1, 1 o 2');
                throw(error)                
            end
            if this.pParOpt.GeneraPlanOptimoComoBase && ~isempty(this.PlanOptimo)
            ...
            for nro_cadena = 1:cantidad_cadenas
                ...
                if this.iNivelDebug > 1
                    prot.imprime_texto(['Inicializa cadena ' num2str(nro_cadena)]);
                end
                
                for etapa = 1:cantidad_etapas
                    % incorpora proyectos gx etapas anteriores
                    proy_agregar = pPlan.entrega_proyectos_gx_acumulados(etapa-1);
                    if this.iNivelDebug > 2
                        prot.imprime_texto('');
                        texto_proy = '';
                        for kk = 1:length(proy_agregar)
                            texto_proy = [texto_proy ' ' num2str(proy_agregar(kk))];
                        end
                        prot.imprime_texto(['Proyectos de Gx a agregar al sep en etapa ' num2str(etapa) ': ' texto_proy]);
                    end
                    
                    for j = 1:length(proy_agregar)
                        proyecto = this.pAdmProy.entrega_proyecto_gx(proy_agregar(j));
                    ...
                    % transmision
                    proy_agregar = pPlan.entrega_proyectos_tx_acumulados(etapa-1);
                    if this.iNivelDebug > 2
                        texto_proy = '';
                        for kk = 1:length(proy_agregar)
                            texto_proy = [texto_proy ' ' num2str(proy_agregar(kk))];
                        end
                        prot.imprime_texto(['Proyectos de Tx a agregar al sep en etapa ' num2str(etapa) ': ' texto_proy]);
                    end
                    
                    for j = 1:length(proy_agregar)
                        proyecto = this.pAdmProy.entrega_proyecto_tx(proy_agregar(j));
                        if ~cadenas{nro_cadena}.sep_actuales{etapa}.agrega_proyecto(proyecto)
                            % Error (probablemente de programación). 
                            texto = ['Error de programacion. Plan ' num2str(pPlan.entrega_no()) ' no pudo ser implementado en SEP en etapa ' num2str(etapa_previa)];
                            error = MException('cOptMCMC:inicializa_cadenas',texto);
                    ...
                    grupos_proy = this.pAdmProy.entrega_proyectos_tx_obligatorios_por_etapa(1, etapa); %1 indica el escenario. Por ahora sólo un escenario
                    if estrategia_genera_planes(nro_cadena) ~= 0
                        if ~isempty(grupos_proy)
                            for j = 1:length(grupos_proy)
                                ...
                                [proy_ppal, proy_conect] = this.selecciona_proyectos_obligatorios_tx(grupos_proy(j).Proyectos, cadenas{nro_cadena}.plan_actual);
                                for k = 1:length(proy_conect)
                                    pPlan.agrega_proyecto_tx(etapa, proy_conect(k).entrega_indice());
                                    cadenas{nro_cadena}.sep_actuales{etapa}.agrega_proyecto(proy_conect(k));
                                end
                                pPlan.agrega_proyecto_tx(etapa, proy_ppal.entrega_indice());
                                ...
                                cadenas{nro_cadena}.CapacidadDecisionesTx(id_decision, etapa:end) = cadenas{nro_cadena}.CapacidadDecisionesTx(id_decision, etapa:end) + delta_capacidad;
                                if this.iNivelDebug > 2
                                    prot.imprime_texto('');
                                    texto_proy = num2str(proy_ppal.entrega_indice());
                                    
                                    for kk = 1:length(proy_conect)
                                        texto_proy = [texto_proy ' ' num2str(proy_agregar(kk))];
                                    end
                                    prot.imprime_texto(['Proyecto obligatorio de Tx a agregar al sep en etapa ' num2str(etapa) ': ' texto_proy]);
                                end
                            end
                        end
                    end
                    
                    % genera plan de expansión generación
                    cant_proyectos = length(this.pAdmProy.ProyGeneracion);
                    prob_construir = this.pParOpt.ProbConstruirPlanBaseGeneracion/cantidad_etapas;
                    cant_total = round(this.CantDecisionesGx/prob_construir,0);
                    espacio_dec_primarias = [1:1:this.CantDecisionesGx zeros(1,cant_total-this.CantDecisionesGx)];
                    ...
                    for i = 1:length(indice_dec_primaria)
                        ...
                        id_proy = this.pAdmProy.entrega_id_proyectos_gx_dado_id_decision(dec_prim);
                        % verifica si algún proyecto se encuentra en plan
                        [proy_realizado, etapa_proy] = pPlan.entrega_ultimo_proyecto_gx_realizado_de_grupo(id_proy);
                        if etapa_proy == 0
                            % ningún proyecto realizado. Se agrega el primero
                            estado_inicial = this.pAdmProy.entrega_estado_inicial_gx(dec_prim);
                            id_proy_salientes = this.pAdmProy.entrega_id_proyectos_gx_salientes_por_indice_decision_y_estado(dec_prim, estado_inicial);
                        else
                            estado_conducente = this.pAdmProy.entrega_proyecto_gx(proy_realizado).entrega_estado_conducente();
                            id_proy_salientes = this.pAdmProy.entrega_id_proyectos_gx_salientes_por_indice_decision_y_estado(dec_prim, estado_conducente);
                        end

                        if ~isempty(id_proy_salientes)
                            % en proyectos de generación hay sólo uno por estado conducente
                            id_proy_seleccionado = id_proy_salientes;
                            pPlan.agrega_proyecto_gx(etapa, id_proy_seleccionado);
                            proy_seleccionado = this.pAdmProy.entrega_proyecto_gx(id_proy_seleccionado);
                            ...
                            cadenas{nro_cadena}.CapacidadDecisionesGx(id_decision, etapa:end) = cadenas{nro_cadena}.CapacidadDecisionesGx(id_decision, etapa:end) + delta_capacidad;
                        end
                    end
                    
                    % genera plan de expansión transmisión
                    if estrategia_genera_planes(nro_cadena) == 2
                        ...
                        cant_total = round(this.CantDecisionesTxPrimarias/prob_construir,0);
                        espacio_dec_primarias = [1:1:this.CantDecisionesTxPrimarias zeros(1,cant_total-this.CantDecisionesTxPrimarias)];
                        decision = espacio_dec_primarias(ceil(rand(cant_proyectos,1)*length(espacio_dec_primarias)));
                        indice_dec_primaria = decision(decision > 0);
                        for i = 1:length(indice_dec_primaria)
                            dec_prim = indice_dec_primaria(i);
                            id_proy = this.pAdmProy.entrega_id_proyectos_tx_primarios_por_indice_decision(dec_prim);
                            % verifica si algún proyecto se encuentra en plan
                            [proy_realizado, etapa_proy] = pPlan.entrega_ultimo_proyecto_tx_realizado_de_grupo(id_proy);
                            if etapa_proy == 0
                                % ningún proyecto realizado. Se agrega el
                                % primero
                                estado_inicial = this.pAdmProy.entrega_estado_inicial_tx_decision_primaria(dec_prim);
                                id_proy_salientes = this.pAdmProy.entrega_id_proyectos_tx_salientes_por_indice_decision_y_estado(dec_prim, estado_inicial(1), estado_inicial(2));
                            else
                                estado_conducente = this.pAdmProy.entrega_proyecto_tx(proy_realizado).entrega_estado_conducente();
                                id_proy_salientes = this.pAdmProy.entrega_id_proyectos_tx_salientes_por_indice_decision_y_estado(dec_prim, estado_conducente(1), estado_conducente(2));
                            end
                            ...
                            if ~isempty(id_proy_salientes)
                                ...
                                pPlan.agrega_proyecto_tx(etapa, id_proy_seleccionado);
                                proy_seleccionado = this.pAdmProy.entrega_proyecto_tx(id_proy_seleccionado);
                                cadenas{nro_cadena}.sep_actuales{etapa}.agrega_proyecto(proy_seleccionado);
                    ...
                    if estrategia_genera_planes(nro_cadena) == 0
                        % genera plan óptimo
                        % generación
                        id_proy_plan_optimo = this.PlanOptimo.entrega_proyectos_gx(etapa);
                        proy_plan_optimo = this.pAdmProy.ProyGeneracion(id_proy_plan_optimo);
                        ...
                        % transmisión
                        id_proy_plan_optimo = this.PlanOptimo.entrega_proyectos_tx(etapa);
                    ...
                    % inicializa opf y evalua sep actual en etapa
                    ...
                    while ~pPlan.es_valido(etapa)
                        ...
                        % ENS
                        [proy_candidatos_ens, ~] = this.determina_espacio_busqueda_repara_plan(pPlan, cadenas{nro_cadena}.sep_actuales{etapa}.entrega_evaluacion(), etapa, 1); % 1 quiere decir que es para ENS
                        proy_agregado = false;
                        if ~isempty(proy_candidatos_ens)
                            ...
                            if this.iNivelDebug > 2
                                ...
                                texto_candidatos = '';
                                for kk = 1:tope
                                    texto_candidatos = [texto_candidatos ' ' num2str(proy_candidatos_ens(kk))];
                                end
                                prot.imprime_texto(['Proyectos candidatos: ' texto_candidatos]);
                            end
                            
                            for i = 1:tope
                                id_proy_selec = proy_candidatos_ens(i);
                                proy_seleccionado = this.pAdmProy.entrega_proyecto_tx(id_proy_selec);
                                cadenas{nro_cadena}.sep_actuales{etapa}.agrega_proyecto(proy_seleccionado);
                                pOPF.calcula_despacho_economico();
                                ...
                                if this.iNivelDebug > 2
                                    sep_debug = cadenas{nro_cadena}.sep_actuales{etapa}.crea_copia();
                                    OPF_debug = cDCOPF(sep_debug, this.pAdmSc, this.pParOpt);
                                ...
                                if this.iNivelDebug > 2
                                    ...
                                    prot.imprime_texto(['Proyecto evaluado: ' num2str(id_proy_selec)]);
                                    prot.imprime_texto('Elementos sobrecargados despues de intento');
                                    texto = sprintf('%-25s %-10s', 'Elemento', 'Carga');
                                    ...
                                    for j = 1:length(elem_flujo_max)
                                        texto = sprintf('%-25s', elem_flujo_max(j).entrega_nombre());
                                        texto = [texto sprintf('%-10s', num2str(round(max(abs(evaluacion_opf.entrega_flujo_linea(elem_flujo_max(j))))/elem_flujo_max(j).entrega_sr(),2)))];
                                        prot.imprime_texto(texto);
                                    end
                                end
                            ...
                            if this.iNivelDebug > 1
                                ...
                                prot.imprime_texto(['Mejor proyecto evaluado: ' num2str(id_proy_seleccionado)]);
                                prot.imprime_texto('Elementos sobrecargados despues de intento');
                                ...
                                for j = 1:length(elem_flujo_max)
                                    texto = sprintf('%-25s', elem_flujo_max(j).entrega_nombre());
                                    texto = [texto sprintf('%-10s', num2str(round(max(abs(eval_actual.entrega_flujo_linea(elem_flujo_max(j))))/elem_flujo_max(j).entrega_sr(),2)))];
                                    prot.imprime_texto(texto);
                                end
                            end
                        ...
                        % Recorte RES
                        [proy_candidatos_recorte, ~] = this.determina_espacio_busqueda_repara_plan(pPlan, pOPF.entrega_evaluacion(), etapa, 2); % 2 quiere decir que es para recorte RES
                        if ~isempty(proy_candidatos_recorte)
                            ...
                            if this.iNivelDebug > 2
                                ...
                                texto_candidatos = '';
                                for kk = 1:tope
                                    texto_candidatos = [texto_candidatos ' ' num2str(proy_candidatos_recorte(kk))];
                                end
                                prot.imprime_texto(['Proyectos candidatos: ' texto_candidatos]);
                            end
                ...
                [proy, etapas] = pPlan.entrega_proyectos_tx_y_etapas();
                cadenas{nro_cadena}.ProyTransmision(1,proy) = etapas;
                [proy, etapas] = pPlan.entrega_proyectos_gx_y_etapas();
                cadenas{nro_cadena}.ProyGeneracion(1,proy) = etapas;
                ...
                cadenas{nro_cadena}.FObj(1) = pPlan.entrega_totex_total();
...
%                cadenas{nro_cadena}.IntercambioCadena(1) = 0;
            ...
            for corr = 1:this.CantDecisionesTxPrimarias
                this.CapacidadDecisionesTxPorCadenas{corr} = zeros(cantidad_cadenas, cantidad_etapas);
                for cad = 1:cantidad_cadenas
                    this.CapacidadDecisionesTxPorCadenas{corr}(cad,:) = cadenas{cad}.CapacidadDecisionesTx(corr,:);
                end
            end
...
%                    cadenas{nro_cadena}.Sigma(paso_actual,:) = cadenas{nro_cadena}.SigmaActual;
                ...
                plan_prueba = cPlanExpansionGxTx(pPlan.entrega_no() + 1);
                plan_prueba.ProyTransmision = pPlan.ProyTransmision;
                plan_prueba.EtapasTransmision = pPlan.EtapasTransmision;
                ...
                if nivel_debug > 2
                    this.debug_verifica_capacidades_corredores(plan_prueba, cadenas{nro_cadena}.CapacidadDecisionesTx, 'Punto verificacion 1');
                end
                ...
                [proyectos_modificar, etapas_originales, nuevas_etapas, capacidades_plan_prueba]= this.modifica_plan(plan_prueba, nro_cadena, cadenas{nro_cadena}.CapacidadDecisionesTx);
                    ...
                    for nro_etapa = desde_etapa:hasta_etapa
                        for jj = 1:length(proyectos_modificar)
                            if etapas_originales(jj) < cantidad_etapas + 1 && ...
                                ...
                                proyecto = this.pAdmProy.entrega_proyecto_tx(proyectos_modificar(jj));
                        ...
                        while ~plan_prueba.es_valido(nro_etapa)
                            ...
                            % ENS
                            [candidatos_ens, etapas_ens] = this.determina_espacio_busqueda_repara_plan(plan_prueba, cadenas{nro_cadena}.sep_actuales{nro_etapa}.entrega_opf().entrega_evaluacion(), nro_etapa, 1); % 1: ENS
                            proy_agregado = false;                            
                            if ~isempty(candidatos_ens)
                                ...
                                if etapas_cand_ens(mejor_intento) == cantidad_etapas+1
                                    plan_prueba.agrega_proyecto_tx(nro_etapa, id_proy_seleccionado);
                                    if nivel_debug > 1
                                        prot.imprime_texto(['ENS: Se agrega proyecto ' num2str(id_proy_seleccionado) ' a etapa ' num2str(nro_etapa)]);
                                    end
                                else
                                    plan_prueba.adelanta_proyectos_tx(id_proy_seleccionado, etapa_seleccionado, nro_etapa);
                                    if nivel_debug > 1
                            ...
                            % recorte RES
                            [candidatos_recorte, etapas_recorte] = this.determina_espacio_busqueda_repara_plan(plan_prueba, cadenas{nro_cadena}.sep_actuales{nro_etapa}.entrega_opf().entrega_evaluacion(), nro_etapa, 2); % 2: recorte RES
                ...
                if existe_cambio_global
                    if plan_prueba.entrega_totex_total() <= pPlan.entrega_totex_total()
                        ...
                        f_obj_prueba = plan_prueba.entrega_totex_total();
                        f_obj_actual = pPlan.entrega_totex_total();
                ...
                if acepta_cambio
                    if nivel_debug > 1
                        ...
                        prot.imprime_texto(['Totex actual (f objetivo): ' num2str(pPlan.entrega_totex_total()) ' (' num2str(pPlan.entrega_totex_total()) ')']);
                        prot.imprime_texto(['Totex nuevo (f objetivo): ' num2str(plan_prueba.entrega_totex_total()) ' (' num2str(plan_prueba.entrega_totex_total()) ')']);
                    end
                    % se guarda nuevo plan en cadena
                    pPlan = plan_prueba;
                    cadenas{nro_cadena}.plan_actual = plan_prueba;
                    cadenas{nro_cadena}.CambiosEstado(paso_actual) = 1;
                    cadenas{nro_cadena}.ProyTransmision(paso_actual,:) = cadenas{nro_cadena}.ProyTransmision(paso_actual-1,:);
                    cadenas{nro_cadena}.CapacidadDecisionesTx = capacidades_plan_prueba;
                    for ii = 1:length(proyectos_cambiados_prueba)
                        if etapas_nuevas_plan_prueba(ii) <= cantidad_etapas
                            cadenas{nro_cadena}.ProyTransmision(paso_actual,proyectos_cambiados_prueba(ii)) = etapas_nuevas_plan_prueba(ii);
                        else
                            cadenas{nro_cadena}.ProyTransmision(paso_actual,proyectos_cambiados_prueba(ii)) = 0;
                        end
                    end
                    cadenas{nro_cadena}.Totex(paso_actual) = pPlan.entrega_totex_total();                        
                    cadenas{nro_cadena}.FObj(paso_actual) = pPlan.entrega_totex_total();
                     % estadística
                    if pPlan.entrega_totex_total() < cadenas{nro_cadena}.MejorTotex
                        cadenas{nro_cadena}.MejorTotex = pPlan.entrega_totex_total();
                        cadenas{nro_cadena}.MejorFObj = pPlan.entrega_totex_total();
                    ...
                    if nivel_debug > 1
                        ...
                        prot.imprime_texto(['Totex actual (f objetivo): ' num2str(pPlan.entrega_totex_total()) '(' num2str(pPlan.entrega_totex_total()) ')']);
                        prot.imprime_texto(['Totex no aceptado (f objetivo): ' num2str(plan_prueba.entrega_totex_total()) '(' num2str(plan_prueba.entrega_totex_total()) ')']);
                    end
                        ...
                        for jj = 1:length(proyectos_cambiados_prueba)
                            if etapas_nuevas_plan_prueba(jj) < cantidad_etapas + 1 && ...
                               ...
                               nro_etapa < etapas_originales_plan_actual(jj) 
                                % proyecto se retrasa, por lo que hay que eliminarlo del SEP
                                proyecto = this.pAdmProy.entrega_proyecto_tx(proyectos_cambiados_prueba(jj));
                                cadenas{nro_cadena}.sep_actuales{nro_etapa}.elimina_proyecto(proyecto);
                ...
                if nivel_debug > 1
                    prot.imprime_matriz([cadenas{nro_cadena}.ProyTransmision(paso_actual-1,:); cadenas{nro_cadena}.ProyTransmision(paso_actual,:)], 'Matriz proyectos pasos anterior y actual');
...
%                proyectos_optimizar = this.selecciona_proyectos_optimizar(plan_prueba, proyectos_modificar);
                ...
                while indice_optimizar_actual < length(proyectos_optimizar)
                    ...
                    plan_actual.ProyTransmision = plan_prueba.ProyTransmision; % plan_actual.ProyTransmision; plan_actual.EtapasTransmision;
                    plan_actual.EtapasTransmision = plan_prueba.EtapasTransmision; % plan_actual.ProyTransmision; plan_actual.Etapas;
                    totex_mejor_etapa = plan_prueba.entrega_totex_total();
                    mejor_etapa = 0;
                    
                    % modifica sep y evalua plan a partir de primera etapa cambiada
                    desde_etapa = plan_prueba.entrega_etapa_proyecto_tx(proy_seleccionado, true); % true indica que entrega error si proyecto seleccionado no está en plan
                    proyecto = this.pAdmProy.entrega_proyecto_tx(proy_seleccionado);

                    if desde_etapa == 0
                        % No entra aquí, ya que antes indica un error. Por ahora se mantiene esta parte del código por si a futuro se consideran también proyectos que no están en el plan. Sin embargo, no debiera entrar aquí
                        ...
                        proy_aguas_arriba = this.pAdmProy.entrega_id_proyectos_tx_salientes_por_indice_decision_y_estado(id_decision, estado_conducente(1), estado_conducente(2));
                        ultima_etapa_posible = plan_prueba.entrega_ultima_etapa_posible_modificacion_proyecto_tx(proy_aguas_arriba, desde_etapa)-1;
                    ...
                    for nro_etapa = desde_etapa:ultima_etapa_posible
                        ...
                        if nro_etapa < cantidad_etapas
                            plan_prueba.desplaza_proyectos_tx(proy_seleccionado, nro_etapa, nro_etapa + 1);
                        else
                            plan_prueba.elimina_proyectos_tx(proy_seleccionado, nro_etapa);
                        end
                        ...
                        if plan_prueba.es_valido(nro_etapa) && plan_prueba.entrega_totex_total() < totex_mejor_etapa
                            ...
                            plan_actual_mejor_etapa.ProyTransmision = plan_prueba.ProyTransmision;
                            plan_actual_mejor_etapa.EtapasTransmision = plan_prueba.EtapasTransmision;
                        ...
                        elseif ~plan_prueba.es_valido(nro_etapa)
                            ...
                            if nro_etapa < cantidad_etapas
                                delta_cinv_proyectado = this.calcula_delta_cinv_elimina_proyectos_tx(plan_prueba, nro_etapa+1, proy_seleccionado);
                    ...
                    plan_prueba.ProyTransmision = plan_actual.ProyTransmision;
                    plan_prueba.EtapasTransmision = plan_actual.EtapasTransmision;
                    ...
                    if desde_etapa > 1
                        ...
                        if proyecto.TieneDependencia
                            [~, primera_etapa_posible]= plan_prueba.entrega_proyecto_tx_dependiente(proyecto.entrega_indices_proyectos_dependientes(), false);
                        ...
                        while nro_etapa > primera_etapa_posible && ~flag_salida
                            ...
                            cadenas{nro_cadena}.sep_actuales{nro_etapa}.agrega_proyecto(proyecto);
                            if nro_etapa == cantidad_etapas
                                plan_prueba.agrega_proyecto_tx(nro_etapa, proy_seleccionado);
                            else
                                plan_prueba.adelanta_proyectos_tx(proy_seleccionado, nro_etapa + 1, nro_etapa);
                            end
                            ...
                            elseif plan_prueba.es_valido(nro_etapa)
                                ...
                                delta_cinv_proyectado = this.calcula_delta_cinv_adelanta_proyectos_tx(plan_prueba, nro_etapa, proy_seleccionado);
                                delta_coper_proyectado = this.estima_delta_coper_adelanta_proyectos_tx(nro_etapa, delta_coper_actual_adelanta, delta_coper_anterior_adelanta);
                    ...
                    if mejor_etapa ~=0
                        existe_cambio_global = true;
                        plan_prueba.ProyTransmision = plan_actual_mejor_etapa.ProyTransmision;
                        plan_prueba.EtapasTransmision = plan_actual_mejor_etapa.EtapasTransmision;
                ...
                if nivel_debug > 1
                    ...
                    if ~existe_cambio_global
                        ...
                        for ii = 1:length(proyectos_cambiados_prueba)
                            ...
                            etapa_orig = pPlan.entrega_etapa_proyecto_tx(proy_orig, false);
                            nueva_etapa = plan_prueba.entrega_etapa_proyecto_tx(proy_orig, false);
...
%                    for nro_etapa = desde_etapa:hasta_etapa
                ...
                if nivel_debug > 1
                    ...
                    plan_expansion_plan_base.ProyTransmision = plan_prueba.ProyTransmision;
                    plan_expansion_plan_base.EtapasTransmision = plan_prueba.EtapasTransmision;
                ...
                evaluacion_actual = plan_prueba.entrega_evaluacion();
                estructura_costos_actual = plan_prueba.entrega_estructura_costos();
                plan_actual.ProyTransmision = plan_prueba.ProyTransmision;
                plan_actual.EtapasTransmision = plan_prueba.EtapasTransmision;
...
%                       proy_seleccionados.directo = 0/1
                        ...
                        intentos_actuales{intento_paralelo_actual}.Valido = false;
                        intentos_actuales{intento_paralelo_actual}.ProyTransmision = [];
                        intentos_actuales{intento_paralelo_actual}.EtapasTransmision = [];
                        ...
                        for nro_etapa = desde_etapa:hasta_etapa
                            ...
                            for k = length(proyectos_eliminar):-1:1
                                if etapas_eliminar(k) <= nro_etapa
                                    proyecto = this.pAdmProy.entrega_proyecto_tx(proyectos_eliminar(k));
                                    cadenas{nro_cadena}.sep_actuales{nro_etapa}.elimina_proyecto(proyecto);
                                    if nro_etapa < cantidad_etapas
                                        plan_prueba.desplaza_proyectos_tx(proyectos_eliminar(k), nro_etapa, nro_etapa + 1);
                                    else
                                        plan_prueba.elimina_proyectos_tx(proyectos_eliminar(k), nro_etapa);
                                    end
                                end
                            end
                            %desplaza proyectos
                            for k = length(proyectos_desplazar):-1:1
                                if nro_etapa >= etapas_originales_desplazar(k) && nro_etapa < etapas_desplazar(k) 
                                    proyecto = this.pAdmProy.entrega_proyecto_tx(proyectos_desplazar(k));
                                    cadenas{nro_cadena}.sep_actuales{nro_etapa}.elimina_proyecto(proyecto);
                                    plan_prueba.desplaza_proyectos_tx(proyectos_desplazar(k), nro_etapa, nro_etapa + 1);
                                end
                            end
                            ...
                            if plan_prueba.es_valido(nro_etapa) && plan_prueba.entrega_totex_total() < estructura_costos_actual_intento.TotexTotal
                                ...
                                % sigue evaluando
                                % acepta y se guarda
								plan_actual_intento.ProyTransmision = plan_prueba.ProyTransmision;
								plan_actual_intento.EtapasTransmision = plan_prueba.EtapasTransmision;
                            ...
                            elseif ~plan_prueba.es_valido(nro_etapa)
                                ...
                                intentos_actuales{intento_paralelo_actual}.SinMejoraIntermedia = true;
                                if nro_etapa < cantidad_etapas
                                    delta_cinv = this.calcula_delta_cinv_elimina_desplaza_proyectos_tx(plan_prueba, nro_etapa+1, proyectos_eliminar, proyectos_desplazar, etapas_originales_desplazar, etapas_desplazar);
                        ...
                        if existe_mejora
                            intentos_actuales{intento_paralelo_actual}.ProyTransmision = plan_actual_intento.ProyTransmision;
                            intentos_actuales{intento_paralelo_actual}.EtapasTransmision = plan_actual_intento.EtapasTransmision;
                            ...
                            while nro_etapa > proy_adelantar.primera_etapa_posible && ~flag_salida
                                ...
                                for k = 1:length(proy_adelantar.proy_conect_adelantar)
                                    if nro_etapa < proy_adelantar.etapas_orig_conect(k) 
                                        proyecto = this.pAdmProy.entrega_proyecto_tx(proy_adelantar.proy_conect_adelantar(k));
                                        cadenas{nro_cadena}.sep_actuales{nro_etapa}.agrega_proyecto(proyecto);
                                        plan_prueba.adelanta_proyectos_tx(proy_adelantar.proy_conect_adelantar(k), nro_etapa + 1, nro_etapa);
                                    end
                                end
                                proyecto = this.pAdmProy.entrega_proyecto_tx(proy_adelantar.seleccionado);
                                cadenas{nro_cadena}.sep_actuales{nro_etapa}.agrega_proyecto(proyecto);
                                plan_prueba.adelanta_proyectos_tx(proy_adelantar.seleccionado, nro_etapa + 1, nro_etapa);
                                ...
                                if ~existe_resultado_adelanta
                                    ...
                                    if plan_prueba.entrega_totex_total() < estructura_costos_actual.TotexTotal
                                        ...
                                        plan_actual_intento_adelanta.ProyTransmision = plan_prueba.ProyTransmision;
                                        plan_actual_intento_adelanta.EtapasTransmision = plan_prueba.EtapasTransmision;
                                ...
                                if existe_mejora_parcial
                                    ...
                                    if ~intentos_actuales{intento_paralelo_actual}.Valido || ...
                                        ...
                                        % se acepta el cambio
                                        intentos_actuales{intento_paralelo_actual}.ProyTransmision = plan_actual_intento_adelanta.ProyTransmision;
                                        intentos_actuales{intento_paralelo_actual}.EtapasTransmision = plan_actual_intento_adelanta.EtapasTransmision;
                    ...
                    if existe_mejora
                        ...
                        plan_actual.ProyTransmision = intentos_actuales{id_mejor_plan_intento}.ProyTransmision;
                        plan_actual.EtapasTransmision = intentos_actuales{id_mejor_plan_intento}.EtapasTransmision;
                        ...
                        if intentos_actuales{id_mejor_plan_intento}.AdelantaProyectos
                            ...
                            proy_ppal = this.pAdmProy.entrega_proyecto_tx(proy_adelantar.seleccionado);                            
                            delta_capacidad = proy_ppal.entrega_capacidad_adicional();
                            id_decision = proy_ppal.entrega_indice_decision_expansion();
                            ...
                            %etapa_nuevas = ultima_etapa_valida_intento;
                            proy_ppal = this.pAdmProy.entrega_proyecto_tx(proy_seleccionados.seleccionado);                            
                            delta_capacidad = proy_ppal.entrega_capacidad_adicional();
                            id_decision = proy_ppal.entrega_indice_decision_expansion();
            ...
            parfor nro_cadena = 1:cantidad_cadenas
                ...
                if acepta_cambio
                    if nivel_debug > 1
                        ...
                        fprintf(doc_id, strcat(texto, '\n'));
                        texto = ['Totex actual (f objetivo): ' num2str(pPlan.entrega_totex_total()) ' (' num2str(pPlan.entrega_totex_total()) ')'];
                        fprintf(doc_id, strcat(texto, '\n'));
                        texto = ['Totex nuevo (f objetivo): ' num2str(plan_prueba.entrega_totex_total()) ' (' num2str(plan_prueba.entrega_totex_total()) ')'];
                        fprintf(doc_id, strcat(texto, '\n'));                        
                    end
                    % se guarda nuevo plan en cadena
                    pPlan = plan_prueba;
                    ...
                    if nivel_debug > 1
                        ...
                        texto = ['Totex actual (f objetivo): ' num2str(pPlan.entrega_totex_total()) '(' num2str(pPlan.entrega_totex_total()) ')'];
                        fprintf(doc_id, strcat(texto, '\n'));
                        texto = ['Totex no aceptado (f objetivo): ' num2str(plan_prueba.entrega_totex_total()) '(' num2str(plan_prueba.entrega_totex_total()) ')'];
                        fprintf(doc_id, strcat(texto, '\n'));
                    end
                    cadenas{nro_cadena}.CambiosEstado(paso_actual) = 0;
                    cadenas{nro_cadena}.ProyTransmision(paso_actual,:) = cadenas{nro_cadena}.ProyTransmision(paso_actual-1,:);
                ...
                while indice_optimizar_actual < length(proyectos_optimizar)
                    ...
                    for nro_etapa = desde_etapa:ultima_etapa_posible
                        ...
                        if plan_prueba.es_valido(nro_etapa) && plan_prueba.entrega_totex_total() < totex_mejor_etapa
                            ...
                            totex_mejor_etapa = plan_prueba.entrega_totex_total();
                            estructura_costos_actual_mejor_etapa = plan_prueba.entrega_estructura_costos();
                            plan_actual_mejor_etapa = struct;
                            plan_actual_mejor_etapa.ProyTransmision = plan_prueba.ProyTransmision;
                            plan_actual_mejor_etapa.EtapasTransmision = plan_prueba.EtapasTransmision;
                            evaluacion_actual_mejor_etapa = plan_prueba.entrega_evaluacion();
        ...
        function cadenas = mapea_espacio_paralelo_bl_detallada(this, cadenas, paso_actual, t_inicio_proceso, nivel_debug)
            ...
            parfor nro_cadena = 1:cantidad_cadenas
                ...
                if nivel_debug > 1
                    ...
                    expansion_plan_base.ProyTransmision = plan_prueba.ProyTransmision;
                    expansion_plan_base.EtapasTransmision = plan_prueba.EtapasTransmision;
                    evaluacion_plan_base = plan_prueba.entrega_evaluacion();
                ...
                expansion_actual.ProyTransmision = plan_prueba.ProyTransmision;
                expansion_actual.EtapasTransmision = plan_prueba.EtapasTransmision;
                cant_busqueda_fallida = 0;
                proy_potenciales_eliminar = []; % no se verifica adelanta
                proy_potenciales_adelantar = []; %también se analiza elimina
                ...
                while cant_busqueda_fallida < cant_fallida
                    ...
                    while intento_paralelo_actual < cant_proy_compara_base || fuerza_continuar_comparacion
                        intento_paralelo_actual = intento_paralelo_actual +1;
                        expansion_actual_intento = expansion_actual;
                        evaluacion_actual_intento = evaluacion_actual;
                        estructura_costos_actual_intento = estructura_costos_actual;
...
%                       proy_seleccionados.directo = 0/1
                        ...
                        for nro_etapa = desde_etapa:hasta_etapa
                            ...
                            if plan_prueba.es_valido(nro_etapa) && plan_prueba.entrega_totex_total() < estructura_costos_actual_intento.TotexTotal
                                ...
                                % sigue evaluando
                                % acepta y se guarda
                                expansion_actual_intento.ProyTransmision = plan_prueba.ProyTransmision;
                                expansion_actual_intento.EtapasTransmision = plan_prueba.EtapasTransmision;
                                estructura_costos_actual_intento = plan_prueba.entrega_estructura_costos();
                                evaluacion_actual_intento = plan_prueba.entrega_evaluacion();
                                existe_mejora = true;
                                plan_actual_intento_hasta_etapa = nro_etapa;
                        ...
                        mejor_totex_elimina_desplaza = inf;
                        if existe_mejora
                            intentos_actuales{intento_paralelo_actual}.ProyTransmision = expansion_actual_intento.ProyTransmision;
                            intentos_actuales{intento_paralelo_actual}.EtapasTransmision = expansion_actual_intento.EtapasTransmision;
                            intentos_actuales{intento_paralelo_actual}.estructura_costos_actual = estructura_costos_actual_intento;
                            intentos_actuales{intento_paralelo_actual}.evaluacion_actual = evaluacion_actual_intento;
                            intentos_actuales{intento_paralelo_actual}.Valido = true;
                            intentos_actuales{intento_paralelo_actual}.Totex = estructura_costos_actual_intento.TotexTotal;
                        ...
                        % se deshace el cambio en el sep
                        plan_prueba.ProyTransmision = expansion_actual.ProyTransmision;
                        plan_prueba.EtapasTransmision = expansion_actual.EtapasTransmision;
                        plan_prueba.inserta_estructura_costos(estructura_costos_actual);
                        plan_prueba.inserta_evaluacion(evaluacion_actual);
                            ...
                            while nro_etapa > proy_adelantar.primera_etapa_posible && ~flag_salida
                                ...
                                if ~existe_resultado_adelanta
                                    ...
                                    if plan_prueba.entrega_totex_total() < estructura_costos_actual.TotexTotal
                                        ...
                                        plan_actual_intento_adelanta = struct;
                                        plan_actual_intento_adelanta.ProyTransmision = plan_prueba.ProyTransmision;
                                        plan_actual_intento_adelanta.EtapasTransmision = plan_prueba.EtapasTransmision;
                                        estructura_costos_actual_intento_adelanta = plan_prueba.entrega_estructura_costos();
                                        evaluacion_actual_intento_adelanta = plan_prueba.entrega_evaluacion();
                    ...
                    if existe_mejora
                        ...
                        expansion_actual.ProyTransmision = intentos_actuales{id_mejor_plan_intento}.ProyTransmision;
                        expansion_actual.EtapasTransmision = intentos_actuales{id_mejor_plan_intento}.EtapasTransmision;
                        evaluacion_actual = intentos_actuales{id_mejor_plan_intento}.evaluacion_actual;
                        estructura_costos_actual = intentos_actuales{id_mejor_plan_intento}.estructura_costos_actual;
                        plan_prueba.ProyTransmision = expansion_actual.ProyTransmision;
                        plan_prueba.EtapasTransmision = expansion_actual.EtapasTransmision;
...
%           parfor nro_cadena = 1:cantidad_cadenas
            ...
            for id_cadena = 1:cantidad_cadenas_principales
                ...
                for id_parametro = 1:cantidad_parametros
                    ...
                    cantidad_cambios = 0;
                    cantidad_etapas = this.pParOpt.CantidadEtapas;
                    etapa_original_parametro = pPlan.entrega_etapa_proyecto_tx(id_parametro, false);
...
throw(error)
                        ...
                        if nivel_debug > 1
                            etapa_orig = pPlan.entrega_etapa_proyecto_tx(id_parametro, false);
                            nueva_etapa = plan_prueba.entrega_etapa_proyecto_tx(id_parametro, false);
                            f_obj_actual = pPlan.entrega_totex_total()-this.NPVCostosOperacionSinRestriccion;
                            f_obj_prueba = plan_prueba.entrega_totex_total()-this.NPVCostosOperacionSinRestriccion;
                            if existe_cambio_global
                                sigma = this.pParOpt.SigmaFuncionLikelihood;
...
throw(error)
                    ...
                    etapa_final_parametro = pPlan.entrega_etapa_proyecto_tx(id_parametro, false);
                        ...
                        for nro_etapa = desde_etapa:hasta_etapa
                            if etapa_final_parametro < cantidad_etapas + 1 && ...
                               ...
                               nro_etapa < etapa_original_parametro
                                % proyecto se retrasa, por lo que hay que eliminarlo del SEP
                                proyecto = this.pAdmProy.entrega_proyecto_tx(id_parametro);
                                cadenas{nro_cadena}.sep_actuales{nro_etapa}.elimina_proyecto(proyecto);
...
throw(error)
                            ...
                            if this.pParOpt.OptimizaEnCalculosTolerancia && existe_cambio_global
                                ...
                                while ~isempty(proyectos_optimizar_prueba)
                                    ...
                                    % modifica sep y evalua plan a partir de primera etapa cambiada
                                    desde_etapa = plan_prueba.entrega_etapa_proyecto_tx(proy_seleccionado, false); % false indica que es sin error. Si proyecto no está en el plan entrega 0
                                    proyecto = this.pAdmProy.entrega_proyecto_tx(proy_seleccionado);
...
throw(error)
        ...
        % 
        %                                     plan_debug = cPlanExpansionGxTx(888888889);
        ...
        %                                 end
                                        ...
                                        if plan_prueba.entrega_totex_total() < totex_mejor_etapa
                                            ...
                                            expansion_actual_mejor_etapa.ProyTransmision = plan_prueba.ProyTransmision;
                                            expansion_actual_mejor_etapa.EtapasTransmision = plan_prueba.EtapasTransmision;
                                            evaluacion_actual_mejor_etapa = plan_prueba.entrega_evaluacion();
...
throw(error)
                                    ...
                                    if mejor_etapa ~=0
                                        existe_cambio_global = true;
                                        plan_prueba.ProyTransmision = expansion_actual_mejor_etapa.ProyTransmision;
                                        plan_prueba.EtapasTransmision = expansion_actual_mejor_etapa.EtapasTransmision;
                                        plan_prueba.inserta_estructura_costos(estructura_costos_actual_mejor_etapa);
                                        plan_prueba.inserta_evaluacion(evaluacion_actual_mejor_etapa);
                                        proyectos_cambiados_prueba = [proyectos_cambiados_prueba proy_seleccionado];
                                        etapas_originales_plan_actual = [etapas_originales_plan_actual desde_etapa];
...
throw(error)
                        ...
                        for nro_etapa = 1:cantidad_etapas
                            ...
                            for jj = 1:length(faltan_en_sep)
                                proyecto = this.pAdmProy.entrega_proyecto_tx(faltan_en_sep(jj));
                                cadenas{nro_cadena}.sep_actuales{nro_etapa}.agrega_proyecto(proyecto);
                            end
                            for jj = 1:length(sobran_en_sep)
                                proyecto = this.pAdmProy.entrega_proyecto_tx(sobran_en_sep(jj));
                                cadenas{nro_cadena}.sep_actuales{nro_etapa}.elimina_proyecto(proyecto);
                            end
                        end                        
...
throw(error)
        ...
        function imprime_estadisticas(this, cadenas, paso_actual, dt_iteracion, dt_acumulado)
            ...
            if paso_actual == 0
                ...
                if ~isempty(this.PlanOptimo)
                    ...
                    texto = sprintf('%-5s %-5s %-10s %-10s %-7s %-7s %-7s %-7s %-7s', ...
        ...
        function [proy_ppal, proy_conectividad]= selecciona_proyectos_obligatorios_tx(this, proy_candidatos, plan)
            % en forma aleatoria
        ...
        function plan = genera_plan_expansion(this, indice)
            plan = cPlanExpansionGxTx(indice);
            cantidad_etapas = this.pParOpt.CantidadEtapas;
            for nro_etapa = 1:cantidad_etapas
...
%                end
        ...
        function crea_plan_optimo(this, data)
            this.PlanOptimo = cPlanExpansionGxTx(9999999);
            [~, m] = size(data.Plan);
        ...
        function crea_plan_evaluar(this, data)
            ...
            this.PlanEvaluar = cPlanExpansionGxTx(9999999);
        ...
        function crea_plan_evaluar_de_cplanexpansiongxtx(this, plan)
            % plan corresponde al plan de expansión
            this.PlanEvaluar = cPlanExpansionGxTx(9999998);
            this.PlanEvaluar.inserta_iteracion(1);
            this.PlanEvaluar.inserta_busqueda_local(false);
            this.PlanEvaluar.ProyTransmision = plan.ProyTransmision;
            this.PlanEvaluar.EtapasTransmision = plan.EtapasTransmision;
            this.PlanEvaluar.agrega_nombre_proyectos(this.pAdmProy);
            this.PlanEvaluar.inserta_sep_original(this.pSEP);            
        end
...
%disp(['Modifica plan cadena ' num2str(nro_cadena)])
            ...
            if this.pParOpt.EstrategiaMCMC == 1
                ...
                corr_modificar_orden = randperm(this.CantDecisionesTxPrimarias);
                cant_corr_modificar = this.pParOpt.CantCorredoresModificar;
                cant_corr_modificados = 0;
                for i = 1:this.CantDecisionesTxPrimarias
                    hubo_cambio = false;
                    corr = corr_modificar_orden(i);
                    cap_corredores = this.CapacidadDecisionesTxPorCadenas{corr}(cadenas_seleccionadas,:);
                        ...
                        if delta_capacidades(etapa) > 0
                            ...
                            while delta_capacidades(etapa) > 0 && ~continua_siguiente_etapa
                                ...
                                if ~isempty(etapa_aumento_capacidad)
                                    ...
                                    id_proy_corr = this.pAdmProy.entrega_id_proyectos_tx_dado_id_decision(corr, id_proy_plan);
                                    if length(id_proy_corr) > 1
                                        % encuentra primer proyecto
                                        % desarrollado
                                        id_proy_corr = plan.entrega_primer_proyecto_tx_realizado_de_grupo_y_etapa(id_proy_corr, etapa_aumento_capacidad);
                                    end                                        
                                    % adelanta proyecto y verifica
                                    % capacidad
                                    plan.adelanta_proyectos_tx(id_proy_corr, etapa_aumento_capacidad, etapa);
                                    hubo_cambio = true;
                                    
                                    capacidad_adicional_proyecto = this.pAdmProy.entrega_proyecto_tx(id_proy_corr).entrega_capacidad_adicional();
                                    ...
                                    id_proy_corr = this.pAdmProy.entrega_id_proyectos_tx_primarios_por_indice_decision(corr);
                                    [proy_realizado, ~] = plan.entrega_ultimo_proyecto_tx_realizado_de_grupo(id_proy_corr);
                                    if proy_realizado ~= 0
                                        estado_conducente = this.pAdmProy.entrega_proyecto_tx(proy_realizado).entrega_estado_conducente();
                                        proy_candidatos = this.pAdmProy.entrega_id_proyectos_tx_salientes_por_indice_decision_y_estado(corr, estado_conducente(1), estado_conducente(2));
                                    else
                                        estado_inicial = this.pAdmProy.entrega_estado_inicial_tx_decision_primaria(corr);
                                        proy_candidatos = this.pAdmProy.entrega_id_proyectos_tx_salientes_por_indice_decision_y_estado(corr, estado_inicial(1), estado_inicial(2));
                                    end
                                    if ~isempty(proy_candidatos)
                                        ...
                                        plan.agrega_proyecto_tx(etapa, proy_seleccionado);
                                        hubo_cambio = true;
                                        capacidad_adicional_proyecto = this.pAdmProy.entrega_proyecto_tx(proy_seleccionado).entrega_capacidad_adicional();
                            ...
                            while delta_capacidades(etapa) < 0 && ~continua_siguiente_etapa
                                ...
                                if etapa > 1
                                    ...
                                    capacidad_inicial = this.pAdmProy.entrega_capacidad_inicial_tx_decision_primaria(corr);
                                ...
                                if existe_aumento
                                    ...
                                    if length(id_proy_corr) > 1
                                        ...
                                        % desarrollado
                                        id_proy_corr = plan.entrega_ultimo_proyecto_tx_realizado_de_grupo_y_etapa(id_proy_corr, etapa);
                                    end
                                    ...
                                    proy_corr = this.pAdmProy.entrega_proyecto_tx(id_proy_corr);
                                    ...
                                    if corresponde
                                        ...
                                        proy_aguas_arriba = this.pAdmProy.entrega_id_proyectos_tx_salientes_por_indice_decision_y_estado(corr, estado_conducente(1), estado_conducente(2));
                                        ultima_etapa_posible = plan.entrega_ultima_etapa_posible_modificacion_proyecto_tx(proy_aguas_arriba, etapa+1);
                                        if ultima_etapa_posible == 0
                                            ultima_etapa_posible = etapa_zero;
                                        end
                                        
                                        etapa_modificar = min(ultima_etapa_con_sentido, ultima_etapa_posible);
                                        if etapa_modificar > cant_etapas
                                            % se elimina proyecto
                                            plan.elimina_proyectos_tx(id_proy_corr, etapa);
                                        else
                                            plan.desplaza_proyectos_tx(id_proy_corr, etapa, etapa_modificar);
                                        end
                                        hubo_cambio= true;
                                        proy_ya_modificado = find(proyectos_seleccionados == id_proy_corr, 1);
                                        if ~isempty(proy_ya_modificado)
...
%                 for i = 1:length(proyectos_seleccionados)
%                     etapas_originales(i) = plan.entrega_etapa_proyecto_tx(proyectos_seleccionados(i), false); % false indica que no entrega error si el proyecto no está en el plan
...
%                         [~, primera_etapa_posible]= plan.entrega_proyecto_tx_dependiente(this.pAdmProy.ProyTransmision(proyectos_seleccionados(i)).entrega_indices_proyectos_dependientes(), false);
%                         if primera_etapa_posible == 0
%                             % proyecto dependiente no está en el plan. No se
%                             % hace nada
%                             nuevas_etapas(i) = etapa_cero;
...
%                             end
%                             plan.agrega_proyecto_tx(nuevas_etapas(i), proyectos_seleccionados(i));
%                         end
%                     else
%                         plan.agrega_proyecto_tx(nuevas_etapas(i), proyectos_seleccionados(i));
%                     end
...
%                         end
%                         
%                         [~, primera_etapa_posible]= plan.entrega_proyecto_tx_dependiente(proy_dependientes);
%                         if primera_etapa_posible > nuevas_etapas(i)
%                             nuevas_etapas(i) = primera_etapa_posible;
%                         end
%                         plan.adelanta_proyectos_tx(proyectos_seleccionados(i), etapas_originales(i), nuevas_etapas(i));
%                     else
%                         plan.adelanta_proyectos_tx(proyectos_seleccionados(i), etapas_originales(i), nuevas_etapas(i));
%                     end
%                 elseif etapas_originales(i) ~= etapa_cero && nuevas_etapas(i) ~= etapa_cero && nuevas_etapas(i) > etapas_originales(i)
...
%                     proy_dep_aguas_arriba = this.pAdmProy.entrega_id_proyectos_tx_salientes(this.pAdmProy.ProyTransmision(proyectos_seleccionados(i)).Elemento(end));
%                     if ~isempty(proy_dep_aguas_arriba)
%                         [~, ultima_etapa_posible]= plan.entrega_proyectos_tx_y_etapa_de_lista(proy_dep_aguas_arriba);
%                     else
...
%                         plan.desplaza_proyectos_tx(proyectos_seleccionados(i), etapas_originales(i), nuevas_etapas(i));
%                     else
...
%                         plan.elimina_proyectos_tx(proyectos_seleccionados(i), etapas_originales(i));
%                     else
%                         nuevas_etapas(i) = ultima_etapa_posible;
%                         plan.desplaza_proyectos_tx(proyectos_seleccionados(i), etapas_originales(i), nuevas_etapas(i));
%                     end
%                 end
        ...
        function [proyectos_seleccionados, etapas_originales, nuevas_etapas] = modifica_parametro(this, plan, id_parametro, sigma)
            ...
            etapa_cero = this.pParOpt.CantidadEtapas + 1;
            etapas_originales = plan.entrega_etapa_proyecto_tx(proyectos_seleccionados, false);
            ...
            elseif etapas_originales == etapa_cero && nuevas_etapas ~= etapa_cero
                ...
                if this.pAdmProy.ProyTransmision(proyectos_seleccionados).TieneDependencia
                    [~, primera_etapa_posible]= plan.entrega_proyecto_tx_dependiente(this.pAdmProy.ProyTransmision(proyectos_seleccionados).entrega_indices_proyectos_dependientes(), false);
                    if primera_etapa_posible == 0
                        % proyecto dependiente no está en el plan. No se
                        % hace nada
                        nuevas_etapas = etapa_cero;
                        ...
                        plan.agrega_proyecto_tx(nuevas_etapas, proyectos_seleccionados);
                    end
                else
                    plan.agrega_proyecto_tx(nuevas_etapas, proyectos_seleccionados);
                end
            elseif etapas_originales ~= etapa_cero && nuevas_etapas ~= etapa_cero && nuevas_etapas < etapas_originales
                ...
                % adelantar
                if this.pAdmProy.ProyTransmision(proyectos_seleccionados).TieneDependencia
                    [~, primera_etapa_posible]= plan.entrega_proyecto_tx_dependiente(this.pAdmProy.ProyTransmision(proyectos_seleccionados).entrega_indices_proyectos_dependientes());
                    if primera_etapa_posible > nuevas_etapas
                        nuevas_etapas = primera_etapa_posible;
                    end
                    plan.adelanta_proyectos_tx(proyectos_seleccionados, etapas_originales, nuevas_etapas);
                else
                    plan.adelanta_proyectos_tx(proyectos_seleccionados, etapas_originales, nuevas_etapas);
                end
            elseif etapas_originales ~= etapa_cero && nuevas_etapas ~= etapa_cero && nuevas_etapas > etapas_originales
                ...
                % una etapa posterior. OJO que eliminé el caso de
                % proyectos obligatorios!!!
                proy_dep_aguas_arriba = this.pAdmProy.entrega_id_proyectos_tx_salientes(this.pAdmProy.ProyTransmision(proyectos_seleccionados).Elemento(end));
                if ~isempty(proy_dep_aguas_arriba)
                    [~, ultima_etapa_posible]= plan.entrega_proyectos_tx_y_etapa_de_lista(proy_dep_aguas_arriba);
                else
                    ultima_etapa_posible = 0;
                ...
                if ultima_etapa_posible == 0
                    % quiere decir que ninguno de los proyectos aguas arriba está en el plan. Se puede retrasar
                    plan.desplaza_proyectos_tx(proyectos_seleccionados, etapas_originales, nuevas_etapas);
                else
                    % hay un proyecto aguas arriba en el plan. Se modifica (eventualmente) etapa a retrasar
            ...
            elseif etapas_originales ~= etapa_cero && nuevas_etapas == etapa_cero
                ...
                if ultima_etapa_posible == 0
                    % proyecto se puede eliminar
                    plan.elimina_proyectos_tx(proyectos_seleccionados, etapas_originales);
                else
                    nuevas_etapas = ultima_etapa_posible;
                    plan.desplaza_proyectos_tx(proyectos_seleccionados, etapas_originales, nuevas_etapas);
                end
        ...
        function proyectos_seleccionados = selecciona_proyectos_optimizar_bl_simple(this, plan, proy_modificados)
            ...
            if ~this.pParOpt.OptimizaCorredoresModificados
                ...
                proy_restringidos = this.pAdmProy.entrega_id_proyectos_tx_dado_id_decision(corr_ya_modificados);
            else
                proy_restringidos = [];
            end
            ...
            elseif this.pParOpt.EstrategiaProyectosOptimizar == 2
                ...
                % luego el resto
                [espacio_busqueda, ~]= this.pAdmProy.determina_espacio_busqueda_elimina_proyectos_tx(plan, this.pParOpt.CantidadEtapas, proy_restringidos);
        ...
        function [escenarios, estados_red] = genera_escenarios_realizacion_plan(this, plan, cantidad_etapas)
            ...
            % escenario_proyectos.ProyTransmision = proy_en_plan;
            ...
            for i = 1:length(proyectos_plan)
                proy = this.pAdmProy.entrega_proyecto_tx(proyectos_plan(i));
            ...
            for etapa = 1:cantidad_etapas
                ...
                for i = 1:m
                    estados_red{etapa}.Estado(i).ProyTransmision = proyectos_plan(estados_unicos(:,i) == 1);
        ...
        function [delta_escenarios, delta_estados_red, delta_etapas] = genera_escenarios_desplaza_proyecto(this, escenarios_orig, estados_red_orig, proy_desplaza, nro_etapa_orig)
            ...
            maximo_retraso = 0;
            id_proy = escenarios_orig{1}.ProyTransmision == proy_desplaza;
            proyectos_plan = escenarios_orig{1}.ProyTransmision;
            for nro_sim = 1:cantidad_simulaciones
            ...
            for d_etapa = 1:delta_etapas+1
                ...
                [~, m] = size(estados_unicos);
                for i = 1:m
                    delta_estados_red{d_etapa}.Estado(i).ProyTransmision = proyectos_plan(estados_unicos(:,i) == 1);
            ...
            for d_etapa = 1:delta_etapas + 1
                for d_estado = 1:length(delta_estados_red{d_etapa}.Estado)
                    proy_d_estado = sort(delta_estados_red{d_etapa}.Estado(d_estado).ProyTransmision);
                    for estado_orig = 1:length(estados_red_orig{nro_etapa_orig + d_etapa-1}.Estado)
                        proy_estado_orig = sort(estados_red_orig{nro_etapa_orig + d_etapa-1}.Estado(estado_orig).ProyTransmision);
        ...
        function [escenarios, estados_red, delta_etapas] = genera_escenarios_adelanta_proyecto(this, escenarios_orig, estados_red_orig, proy_adelanta, nueva_etapa)
            ...
            if nueva_etapa == this.pParOpt.CantidadEtapas
                proyectos_plan = [escenarios{1}.ProyTransmision proy_adelanta];
                id_proy = length(proyectos_plan);
                retrasos_proyecto = zeros(1,cantidad_simulaciones);
                proy = this.pAdmProy.entrega_proyecto_tx(proy_adelanta);
                pbb_retraso = proy.entrega_probabilidad_retraso(); % [0.5 0.3 0.2] pbbs: [sin retraso, 1 año retraso , 2 años retraso]
                suma_acumulada = cumsum(pbb_retraso);
                for j = 1:cantidad_simulaciones
                    retraso = find(rand*suma_acumulada(end)<suma_acumulada, 1, 'first') - 1;
                    ...
                    escenarios{j}.ProyTransmision = proyectos_plan;
                ...
                proyectos_plan = escenarios{1}.ProyTransmision;
            ...
            for etapa = nueva_etapa:ultima_etapa
                ...
                for i = 1:m
					...
					delta_estados_red{d_etapa}.Estado(i).CostosGeneracion = 0;
					delta_estados_red{d_etapa}.Estado(i).ENS = 0;
                    proy_ordenados_delta_estado = sort(delta_estados_red{d_etapa}.Estado(i).ProyTransmision);
                    % verifica si delta estado ya se encuentra en la etapa
                    % y asocia
                    estado_actual_encontrado = 0;
                    for j = 1:cantidad_estados_actuales
                        if isequal(proy_ordenados_delta_estado, sort(estados_red{etapa}.Estado(j).ProyTransmision))
        ...
        function evalua_plan_con_incertidumbre(this, plan)
            ...
            for nro_etapa = 1:this.pParOpt.CantidadEtapas
                ...
                for nro_estado = 1:length(estados_por_etapas{nro_etapa}.Estado)
                    ...
                    proyectos_estado = estados_por_etapas{nro_etapa}.Estado(nro_estado).ProyTransmision;
                    ...
                    for ii = 1:length(proyectos_a_agregar)
                        proyecto = this.pAdmProy.entrega_proyecto_tx(proyectos_a_agregar(ii));
                        sep.agrega_proyecto(proyecto);
                    end
                    for ii = 1:length(proyectos_a_eliminar)
                        proyecto = this.pAdmProy.entrega_proyecto_tx(proyectos_a_eliminar(ii));
        ...
        function estados_por_etapas = determina_estados_por_etapa_plan(this, plan)
            ...
            for i = 1:length(proyectos)
                proy = this.pAdmProy.entrega_proyecto_tx(proyectos(i));
            ...
            for nro_etapa = 1:this.pParOpt.CantidadEtapas
                ...
                if cantidad_estados == 1
                    estados_por_etapas{nro_etapa}.Estado(1).ProyTransmision = proyectos_nodo_base;
                    ...
                    for i = 0:cantidad_estados-1
                        ...
                        estados_por_etapas{nro_etapa}.Estado(i+1).ProyTransmision = [proyectos_nodo_base proyectos_realizados];
                        ...
                        if nro_etapa == 1
                            ...
                            proy_estado_actual = estados_por_etapas{nro_etapa}.Estado(i+1).ProyTransmision; % todos, tanto con y sin incertidumbre
                            for j = 1:length(estados_por_etapas{nro_etapa-1}.Estado)
                                proy_estado_anterior = estados_por_etapas{nro_etapa-1}.Estado(j).ProyTransmision; % todos, tanto con y sin incertidumbre
...
%                 estructura_eval.TrafosPocoUso = evaluacion.entrega_trafos_poco_uso();
        ...
        function calcula_costos_totales(this,plan)
            ...
            for i= 1:length(plan.ProyTransmision) %cantidad etapas
                etapa = plan.EtapasTransmision(i);
                indice = plan.ProyTransmision(i);
                ...
                costo_inv = this.pAdmProy.calcula_costo_inversion_proyecto(this.pAdmProy.entrega_proyecto_tx(indice), etapa, plan, factor_desarrollo);
        ...
        function delta_cinv = calcula_delta_cinv_elimina_proyectos_tx(this, plan, nro_etapa, proyectos_eliminar)
            cinv_actual = plan.CInvTotal;
            
            % "crea" plan objetivo. 
            plan_objetivo = cPlanExpansionGxTx(9999997);
            plan_objetivo.ProyTransmision = plan.ProyTransmision;
            plan_objetivo.EtapasTransmision = plan.EtapasTransmision;

            for k = length(proyectos_eliminar):-1:1
                plan_objetivo.elimina_proyectos_tx(proyectos_eliminar(k), nro_etapa);
            end
        ...
        function delta_cinv = calcula_delta_cinv_adelanta_proyectos_tx(this, plan, nro_etapa, proyectos_adelantar)
            cinv_actual = plan.CInvTotal;
            ...
            for k = length(proyectos_adelantar):-1:1
                plan_objetivo.adelanta_proyectos_tx(proyectos_adelantar(k), nro_etapa,1);
            end
            % calcula costos de inversión de plan_objetivo
            cinv_plan_objetivo = this.calcula_costos_inversion_actual(plan_objetivo);
        ...
        function delta_coper = estima_delta_coper_adelanta_proyectos_tx(this, etapa_actual, delta_coper_actual, delta_coper_anterior)
        ...
        function cinv = calcula_costos_inversion_actual(this,plan)
            ...
            detapa = this.pParOpt.DeltaEtapa;
            cinv = 0;
            for i = 1:length(plan.ProyTransmision)
                etapa = plan.EtapasTransmision(i);
                indice = plan.ProyTransmision(i);
                %costo_inv = this.pAdmProy.ProyTransmision(indice).entrega_costos_inversion();
                factor_desarrollo = this.pParOpt.entrega_factor_costo_desarrollo_proyectos();
        ...
        function valido = evalua_plan_computo_paralelo(this, plan, nro_etapa, puntos_operacion, datos_escenario, sbase)
            ...
            for i = length(proyectos)
                indice = proyectos(i);
                proyecto = this.pAdmProy.entrega_proyecto_tx(indice);
        ...
        function exporta_resultados(this, cadenas)
            ...
            for nro_cadena = 1:cantidad_cadenas
                ...
                [n, cantidad_proyectos] = size(cadenas{nro_cadena}.ProyTransmision);
                ...
                for i = 1:n
                    ...
                    for j = 1:cantidad_proyectos
                        texto_proy = sprintf('%-10s',num2str(cadenas{nro_cadena}.ProyTransmision(i,j)));
        ...
        function [proy_candidatos, etapas] = determina_espacio_busqueda_repara_plan(this, plan, evaluacion, etapa, varargin)
            % varargin indica si es para problemas con ENS (1) o Recorte RES (2)
            % Para proyectos de transmisión, se da prioridad a las congestiones directas. Si no hay proyectos, entonces se empiezan a agregar otros proyectos de líneas congestionadas
            % Para baterías existentes, se agregan proyectos paralelo sólo si tienen uso máximo
            % Por ahora no se consideran proyectos de generación, ya que puede introducir recortes de ernc adicionales
            % etapas indica la etapa en el plan del proyecto candidato. Si aparece un 0 indica que el proyecto candidato no está en el plan y se reemplaza por cant_etapas + 1 al final.
            if nargin > 4
                tipo_reparacion = varargin{1};
                if tipo_reparacion == 1
                    se_con_problemas = evaluacion.entrega_se_con_consumos_ens();
                else
                    se_con_problemas = evaluacion.entrega_se_con_recorte_res();
                end
            else
                tipo_reparacion = 0;
                se_con_problemas = evaluacion.entrega_se_con_consumos_ens_o_recorte_res();
            end
            
            if isempty(se_con_problemas)
                % no hay consumos con ens. Nada que reparar
                proy_candidatos = [];
                etapas = [];
                return
            end
            ...
            if nivel_debug > 1
                if nargin > 4
                    if tipo_reparacion == 1
                        texto_reparacion = ['ENS (' num2str(sum(evaluacion.CostoENS)) ')'];
                    else
                        texto_reparacion = ['Recorte RES (' num2str(sum(evaluacion.CostoRecorteRES)) ')'];
                    end
                else
                    texto_reparacion = ['ENS (' num2str(sum(evaluacion_opf.CostoENS)) ') y recorte RES (' num2str(sum(evaluacion.CostoRecorteRES)) ')'];
                end

                prot = cProtocolo.getInstance;
                prot.imprime_texto(['   Espacio busqueda repara plan debido a ' texto_reparacion '. Elementos sobrecargados:']);
                texto = sprintf('%-25s %-10s %-10s %-30s', 'Elemento', 'Tipo Rep', 'Carga', 'Proyectos');
                prot.imprime_texto(texto);
            end
            
            elem_tx_flujo_max = evaluacion.entrega_lineas_flujo_maximo();
            elem_tx_flujo_max = [elem_tx_flujo_max evaluacion.entrega_trafos_flujo_maximo()];
            elem_flujo_max_revisados = zeros(length(elem_tx_flujo_max),1);
            [baterias_uso_max, id_se_baterias_uso_max] = evaluacion.entrega_baterias_uso_maximo_y_id_se();
            baterias_uso_max_revisadas = zeros(length(baterias_uso_max), 1);
            id_decision_otras_bat_encontrados = [];
            ...
            se_adyacentes_visitadas_nuevas_bat = [];
            if nivel_debug > 1
                texto_imp = cell(length(elem_tx_flujo_max) + length(baterias_uso_max), 4);
                res_elem_tx_flujo_max = evaluacion.entrega_resultado_lineas_y_trafos_flujo_maximo();
                for j = 1:length(elem_tx_flujo_max)
                    texto_imp{j,1} = sprintf('%-25s', elem_tx_flujo_max(j).entrega_nombre());
                    texto_imp{j,2} = sprintf('%-10s', '-');
                    texto_imp{j,3} = sprintf('%-10s', num2str(round(res_elem_tx_flujo_max(j,5),2)));
                    texto_imp{j,4} = sprintf('%-30s', '-');
                end
                pos_bat_desde = length(elem_tx_flujo_max);
                res_bat_uso_max = evaluacion.entrega_resultado_baterias_uso_maximo();            
                for j = 1:length(baterias_uso_max)
                    texto_imp{pos_bat_desde + j,1} = sprintf('%-25s', baterias_uso_max(j).entrega_nombre());
                    texto_imp{pos_bat_desde + j,2} = sprintf('%-10s', '-');
                    texto_imp{pos_bat_desde + j,3} = sprintf('%-10s', num2str(round(res_bat_uso_max(j,5),2)));
                    texto_imp{pos_bat_desde + j,4} = sprintf('%-30s', '-');
                end
                if isempty(elem_tx_flujo_max) && isempty(baterias_uso_max)
                    texto_imp = cell(0,4);
                end
            end
            ...
            for elem = 1:length(se_con_problemas)
                se = se_con_problemas(elem);
                se_adyacentes = cSubestacion.empty;

                conexiones = se.entrega_ultimas_conexiones_elementos_serie();

                id_conexiones = ismember(elem_tx_flujo_max, conexiones) & elem_flujo_max_revisados == 0;
                elem_flujo_max_se = elem_tx_flujo_max(id_conexiones);
                ...
                if isempty(elem_flujo_max_se)
                    % Para este consumo con ens no hay elementos flujos máximos que no hayan sido revisados. Verifica subestaciones adyacentes
                    ...
                    for i = 1:length(se_adyacentes_validas)
                        conexiones = se_adyacentes_validas(i).entrega_ultimas_conexiones_con_se_excluyente(se);

                        id_conexiones = ismember(elem_tx_flujo_max, conexiones) & elem_flujo_max_revisados == 0;
                        nuevos_elem_flujo_max_se = elem_tx_flujo_max(id_conexiones);

                        if ~isempty(nuevos_elem_flujo_max_se)
                            elem_flujo_max_revisados(id_conexiones) = 1;
                            elem_flujo_max_se = [elem_flujo_max_se; nuevos_elem_flujo_max_se];
                ...
                if ~isempty(elem_flujo_max_se)
                    % se obtienen proyectos candidatos
                    for i = 1:length(elem_flujo_max_se)
                        proy = this.pAdmProy.entrega_id_proyectos_tx_salientes(elem_flujo_max_se(i));
                        if ~isempty(proy)
                            etapas_proy = plan.entrega_etapas_implementacion_proyectos_tx_de_lista(proy, etapa+1);
                            id_implementado = find(etapas_proy,1);
                            if ~isempty(id_implementado)
                                % a lo más hay sólo un proyecto de la lista implementado
                                proy = proy(id_implementado);
                                etapas_proy = etapas_proy(id_implementado);
                            end
                            
                            if nivel_debug > 1
                                indice = elem_tx_flujo_max == elem_flujo_max_se(i);
                                texto_proy = '';
                                for k = 1:length(proy)
                                    if k == 1
                ...
                % Baterías
                % prioridades:
                % 1. Baterías conectadas a la subestación con ENS y si no hay, baterías en subestaciones adyacentes:
                %    1.1 adelantar "siguiente" batería implementada en etapa posterior de una que tiene uso máximo
                %    1.2 agregar nueva batería (todas las tecnologías disponibles)
                % 2. Agregar nuevas baterías en otras subestaciones (si punto anterior no entrega proyectos). Eventualmente este
                %    punto se puede restringir a una determinada área de influencia
                %    2.1 adelantar "siguiente" batería implementada en etapa posterior de una que tiene uso máximo
                %    2.2 agregar nueva batería (todas las tecnologías disponibles)
                
                bat_uso_max_se = baterias_uso_max(id_se_baterias_uso_max == se.entrega_id() & baterias_uso_max_revisadas == 0);
                if isempty(bat_uso_max_se)
                    % busca en subestaciones adyacentes
                    if isempty(se_adyacentes)
                        se_adyacentes = se.entrega_subestaciones_adyacentes();
                    end
                    se_adyacentes_validas = se_adyacentes(~ismember(se_adyacentes, se_adyacentes_visitadas_bat));
                    se_adyacentes_visitadas_bat = [se_adyacentes_visitadas_bat se_adyacentes_validas];
                    for i = 1:length(se_adyacentes_validas)
                        id_baterias_validas = id_se_baterias_uso_max == se_adyacentes_validas(i).entrega_id() & baterias_uso_max_revisadas == 0;
                        nuevas_bat_uso_max_no_vistas = baterias_uso_max(id_baterias_validas);
                        if ~isempty(nuevas_bat_uso_max_no_vistas)
                            baterias_uso_max_revisadas(id_baterias_validas) = 1;
                            bat_uso_max_se = [bat_uso_max_se ; nuevas_bat_uso_max_no_vistas];
                            
                            if nivel_debug > 1
                                indices = find(id_baterias_validas == 1);
                                for j = 1:length(indices)
                                    texto_imp{pos_bat_desde + indices(j),2} = sprintf('%-10s', 'Ady');
                                end
                            end
                        end
                    end
                ...
                if ~isempty(bat_uso_max_se)
                    % se obtienen proyectos candidatos
                    for i = 1:length(bat_uso_max_se)
                        proy = this.pAdmProy.entrega_id_proyectos_tx_salientes(bat_uso_max_se(i));
                        if ~isempty(proy)
                            etapas_proy = plan.entrega_etapas_implementacion_proyectos_tx_de_lista(proy, etapa+1);
                            id_implementado = find(etapas_proy,1);
                            if ~isempty(id_implementado)
                            ...
                            if nivel_debug > 1
                                indice = find(baterias_uso_max == bat_uso_max_se(i));
                                texto_proy = '';
                                for k = 1:length(proy)
                                    if k == 1
                                        texto_proy = num2str(proy(k));
                                ...
                                texto_imp{pos_bat_desde + indice,4} = sprintf('%-30s', texto_proy);
                            end
                ...
                % otras baterías conectadas a la subestación o a subestaciones adyacentes que no se encuenten ya conectadas
                baterias_se = this.pAdmProy.entrega_baterias_por_subestacion(se);
                if isempty(baterias_se)
                    % busca nuevas baterías en subestaciones adyacentes
                    if isempty(se_adyacentes)
                        se_adyacentes = se.entrega_subestaciones_adyacentes();
                    end
                    se_adyacentes_validas = se_adyacentes(~ismember(se_adyacentes, se_adyacentes_visitadas_nuevas_bat));
                    se_adyacentes_visitadas_nuevas_bat = [se_adyacentes_visitadas_nuevas_bat se_adyacentes_validas];
                    for i = 1:length(se_adyacentes_validas)
                        nuevas_baterias_se = this.pAdmProy.entrega_baterias_por_subestacion(se_adyacentes_validas(i));
                        nuevas_baterias_se_validas = nuevas_baterias_se(~ismember(nuevas_baterias_se,baterias_uso_max));
                        baterias_se = [baterias_se nuevas_baterias_se_validas];
                    end
                end
                
                for i = 1:length(baterias_se)
                    % se identifican proyectos y se seleccionan aquéllos
                    % que no se han implementado o que se han implementado
                    % en etapas posteriores. Por cada indice de decisión se
                    % selecciona sólo un proyecto (el primero encontrado)
                   
                    id_proy_bat = this.pAdmProy.entrega_id_proyectos_tx_entrantes(baterias_se(i));
                    id_decision_bat = this.pAdmProy.entrega_proyecto_tx(id_proy_bat).entrega_indice_decision_expansion();
                    if ~ismember(id_decision_bat, id_decision_otras_bat_encontrados)
                        etapa_bat = plan.entrega_etapas_implementacion_proyectos_tx_de_lista(id_proy_bat);

                        if etapa_bat == 0 || etapa_bat > etapa
                            % proyecto no implementado o implementado
                            % en etapa posterior (y por ende candidato a adelantarlo)
                            proy_candidatos = [proy_candidatos id_proy_bat];
                            etapas = [etapas etapa_bat];

                            id_decision_otras_bat_encontrados = [id_decision_otras_bat_encontrados id_decision_bat];

                            if nivel_debug > 1
                                [n, ~] = size(texto_imp);
                                texto_imp{n+1,2} = sprintf('%-10s', 'Bateria');
                                texto_imp{n+1,1} = sprintf('%-25s', this.pAdmProy.entrega_proyecto_tx(id_proy_bat).Elemento(end).entrega_nombre());
                ...
                if ~ismember(se.entrega_id(), se_visitadas_nuevos_corr)
                    ...
                    if ~isempty(proy_nuevo_corredor)
                        ...
                        etapas_proy_existente = plan.entrega_etapas_implementacion_proyectos_tx_de_lista(proy_nuevo_corredor, 1);
                        ...
                        if ~isempty(proy_no_implementados)
                            ...
                            if nivel_debug > 1
                                for k = 1:length(proy_no_implementados)
                                    ...
                                    texto_imp{n+1,1} = sprintf('%-25s', this.pAdmProy.entrega_proyecto_tx(proy_no_implementados(k)).Elemento(end).entrega_nombre());
                                    texto_imp{n+1,3} = sprintf('%-10s', '-');
            ...
            if isempty(proy_candidatos)
                % no se encontraron proyetos en las conexiones de las subestaciones ni conexiones adyacentes. Se busca en el resto de los elementos con flujo máximo
                elem_flujo_max_restantes = elem_tx_flujo_max(elem_flujo_max_revisados == 0);
                for j = 1:length(elem_flujo_max_restantes)
                    id_proy_salientes = this.pAdmProy.entrega_id_proyectos_tx_salientes(elem_flujo_max_restantes(j));
                    if ~isempty(id_proy_salientes)

                        etapas_proy_salientes = plan.entrega_etapas_implementacion_proyectos_tx_de_lista(id_proy_salientes, etapa+1);
                        ...
                        if ~isempty(pos_proy_implementado)
                            ...
                            if nivel_debug > 1
                                id = elem_tx_flujo_max == elem_flujo_max_restantes(j);
                    ...
                    % verifica proyectos nuevos corredores en subestaciones adyacentes
            ...
            if ~isempty(proy_candidatos)
                ...
                proy_nuevo_corredor = this.pAdmProy.entrega_id_proyectos_nuevos_corredores_con_se_excluyentes(se_visitadas_nuevos_corr);
                if ~isempty(proy_nuevo_corredor)
                    % existe(n) proyecto(s). Verifica si alguno está implementado
                    etapas_proy_existente = plan.entrega_etapas_implementacion_proyectos_tx_de_lista(proy_nuevo_corredor, 1);
        ...
        function debug_verifica_capacidades_corredores(this, plan, capacidades, texto_falla)
            ...
            cantidad_etapas = this.pParOpt.CantidadEtapas;
            cantidad_decisiones_primarias = this.CantDecisionesTxPrimarias;
            
            [id_proyectos, etapas] = plan.entrega_proyectos_tx_y_etapas();
            id_decision = this.pAdmProy.entrega_id_decision_tx_dado_id_proyectos(id_proyectos);
            ...
            for i = 1:length(id_proyectos)
                ...
                proyecto = this.pAdmProy.entrega_proyecto_tx(id_proyectos(i));
                estado_conducente = proyecto.entrega_estado_conducente();
                capacidad_estado = this.pAdmProy.entrega_capacidad_tx_estado_primario(id_decision_proy, estado_conducente);
...
%                throw(error)
            ...
            plan_debug = cPlanExpansionGxTx(888888889);
            plan_debug.ProyTransmision = plan.ProyTransmision;
            plan_debug.EtapasTransmision = plan.EtapasTransmision;
            plan_debug.inserta_sep_original(this.pSEP);
            for etapa_ii = 1:this.pParOpt.CantidadEtapas
                valido = this.evalua_plan(plan_debug, etapa_ii);
            end
        ...
        function result = selecciona_proyectos_eliminar_desplazar_bl_detallada(this, plan, proy_restringidos, proy_en_evaluacion, varargin)
            ...
            if isempty(result.seleccionado)
                ...
                [espacio_busqueda, desde_etapa] = this.pAdmProy.determina_espacio_busqueda_elimina_proyectos_tx(plan, nro_etapa, [proy_restringidos proy_en_evaluacion]);
                ...
                while isempty(espacio_busqueda) && nro_etapa > 0
                    ...
                    nro_etapa = nro_etapa - 1;
                    [espacio_busqueda, desde_etapa, ultima_etapa_posible] = this.pAdmProy.determina_espacio_busqueda_desplaza_proyectos_tx(plan, nro_etapa, proy_restringidos, true, proy_en_evaluacion); % true indica que son acumulados
                ...
                result.etapa_seleccionado = plan.entrega_etapa_proyecto_tx(indice);
            ...
            if this.pAdmProy.ProyTransmision(indice).TieneRequisitosConectividad
                ...
                for ii = 1:cantidad_req_conectividad
                    ...
                    elseif length(id_proyectos_misma_conectividad) >1
                        ...
                        etapa_proyecto_seleccionado = plan.entrega_etapa_proyecto_tx(indice);
                        etapa_proyecto_conectividad = plan.entrega_etapa_proyecto_tx(id_proyecto_conectividad);
        ...
        function delta_cinv = calcula_delta_cinv_elimina_desplaza_proyectos_tx(this, plan, nro_etapa, proyectos_eliminar, proyectos_desplazar, etapas_originales_desplazar, etapas_desplazar)
            cinv_actual = plan.CInvTotal;
            ...
            for k = length(proyectos_desplazar):-1:1
                if nro_etapa < etapas_originales_desplazar(k)
                    ...
                    plan_objetivo.desplaza_proyectos_tx(proyectos_desplazar(k), etapas_originales_desplazar(k), etapas_desplazar(k));
                elseif nro_etapa >= etapas_originales_desplazar(k) && nro_etapa < etapas_desplazar(k)
                    ...
                    % aún no termina
                    plan_objetivo.desplaza_proyectos_tx(proyectos_desplazar(k), nro_etapa, etapas_desplazar(k));
                else
        ...
        function result = selecciona_proyectos_a_adelantar(this, plan, nro_etapa, varargin)
            ...
            if this.pAdmProy.ProyTransmision(indice).TieneDependencia
                [~, result.primera_etapa_posible]= plan.entrega_proyecto_tx_dependiente(this.pAdmProy.ProyTransmision(indice).entrega_indices_proyectos_dependientes());
            else
